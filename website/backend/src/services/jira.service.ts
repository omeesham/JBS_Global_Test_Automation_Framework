...
// --- DB helpers ---
export async function getCredsForUser(username: string): Promise<JiraCreds | null> {
  const { rows } = await pool.query(
    `SELECT jira_url, auth_header FROM "JBSTestOpsAI".jira_connections WHERE username = $1`,
    [username]
  );
  if (rows.length === 0) return null;
  return { baseUrl: rows[0].jira_url, authHeader: rows[0].auth_header };
}

export async function saveCredsForUser(
  username: string,
  jiraUrl: string,
  authHeader: string,
  displayName?: string
  ...
  await pool.query(
    `INSERT INTO "JBSTestOpsAI".jira_connections (username, jira_url, auth_header, display_name)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (username)
     DO UPDATE SET jira_url = $2, auth_header = $3, display_name = $4, created_at = NOW()`,
    [username, jiraUrl, authHeader, displayName || null]
  );
}

export async function deleteCredsForUser(username: string): Promise<void> {
  await pool.query(`DELETE FROM "JBSTestOpsAI".jira_connections WHERE username = $1`, [username]);
}

export async function getConnectionStatus(username: string) {
  const { rows } = await pool.query(
    `SELECT jira_url, display_name, created_at FROM "JBSTestOpsAI".jira_connections WHERE username = $1`,
    [username]
  );
  if (rows.length === 0) return { connected: false };
  return {
    connected: true,
    jiraUrl: rows[0].jira_url,
...
export default { testConnection, getStories, getStory, getCredsForUser, saveCredsForUser, deleteCredsForUser, getConnectionStatus };
