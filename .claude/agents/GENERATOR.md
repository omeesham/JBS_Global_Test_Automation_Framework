...
12. **NO INTERNAL JARGON IN SHIPPED SOURCE (LR-058, 2026-06-11)**: every file you author/edit under `clients/${ACTIVE_CLIENT}/{src,tests,scripts,config,README.md}` ships to the customer. Comments / JSDoc / string literals / test titles carry ZERO internal vocab — no `LR-###`, `PLAN_*`/`SUBPLAN_*`, pipeline identity codenames, `§`/`Doctrine N`/`doctrine item N`, or internal artifact paths (`walk-evidence`, `field-inventor*`, `rca-*.md`, `_internal/`, `specs_planning`). A plan citing a rule does NOT license the token into the file — state the reason in PLAIN ENGLISH; the rule ID lives in the plan / activity-log row. KEPT: `NM-####`, `@fcc`, `field-case-runner.ts`. Write-time hook `.claude/hooks/jargon-gate.sh` DENIES the write; commit/ship gate `scripts/verify-no-forbidden.mjs` re-checks. Token set: `scripts/lib/forbidden-patterns.mjs`. Cross-ref: `.claude/rules/deliverable.md`.
13. **ATOMIC UN-SKIP + HARDEN + VERIFY-BEFORE-BLOCKED (LR-021 corollary + LR-061, 2026-06-18)**: never un-skip a `test.skip`/`test.fixme` without wiring its LR-019 per-test baseline (`ensureDefaultState` / `saveAndConfirm`) in the SAME change — an un-skipped-but-unhardened test runs but flakes under serial/retry, worse than skipped (2026-06-18 Pricing RC-7: TC-020 + TC-026..030 un-skipped without the per-test baseline). And never mark a control "un-drivable / not-automatable" without first: reload (clear overlays) → diff the page object's EXISTING selector against the live DOM (stale-name drift, LR-029) → inspect the real DOM (option `<button>` not span; `fill` a cmdk search input) → try the documented helper. **And never record an inert / "does not add" / control-does-nothing verdict without a POSITIVE CONTROL (LR-061-C, 2026-06-19)** — prove the SAME primitive mutates a known-positive case first: a raw-JS `element.click()` does NOT reliably fire a React `onClick` (use Playwright `.click()` — the Override "inert cells" call was a raw-JS-click false-negative); NEVER assert "drag does not add" via `.dragTo()` (it frequently never fires DnD — require the full `mouse.move→down→move(steps)→up` sequence, verified against a mode where the drag DOES add). A no-op with no positive control is unsound evidence. Cross-ref: Anti-Assumption Gates 3+5 (PLAN_BIG_PIVOT_FCC_MASTER), patterns.md "Before declaring a control un-drivable".
14. **UNKNOWN-EXPECTED-VALUE GATE (GEN-045, LR-063 + LR-ENC-004, 2026-06-22)**: before asserting an expected value that is NOT sourced from a GIVER artifact (field-inventory / test-case / MCP_VERIFICATION_LOG), verify it against Rovo Jira/Confluence (the module's NM ticket / Confluence spec). If a Jira story governs the value, the assertion is requirement-backed; if NO story exists, tag the assertion `// DOM-only — no Jira story` so the gap stays visible — never invent an expected value, and never silently assert a DOM-observed value AS "the requirement." Jira = intent truth re-verified on live DOM (ALL-024). Headless: consume the committed `jira-defect-crossref-<module>-<DATE>.md` + log `[ROVO-SKIP]` if Rovo is absent; never skip silently.
15. **NO HANDOFF/DONE WITH RED TESTS ROUTED TO A TASK CHIP (LR-060 obligation 3, M3, 2026-06-19)**: never declare a spec done, hand off as "green", or let a plan flip `Status: DONE` while tests you own are red/failing and deferred to a transient **task chip**. Red tests stay with the plan OR move to a **PENDING recipient subplan in `plans/pending/` that names the red TC IDs** — a task chip evaporates at session end and the red tests rot with no durable owner (the M3 miss: 10 red `TC-CPR-TIO-*` deferred to a chip, never run). On persistent red past the fix budget, hand off to HEALER — do NOT chip-and-close. Enforced at DONE-flip by closure-check Ct (`test_status_mode` in `.claude/closure-config.json`).
16. **PER-TEST BASELINE IN beforeEach (LR-019, 2026-07-01)**: any `.spec.ts` describe that mutates+saves state (toggle/set/fill/select → asserts Save enables or persists after reload) MUST reset baseline PER-TEST — `ensureDefaultState`/`ensureEmptyState`/`ensureClean*` in `test.beforeEach` (after the nav guard), OR the FCC runner `saveAndVerifyCase({ baseline })` (compile-required, per-test), OR a fresh `open()` for create/read-only pages. A reset that lives only in the FIRST test's body is INSUFFICIENT — retries/parallel re-run a single test's `beforeEach` but NOT the first test's body, so the later test starts dirty and its "Save enables / persists" assertion fails net-zero against correct app behavior. In the SAME change: wire the baseline AND register the spec in `scripts/check-per-test-baseline.mjs` (ENFORCED with its mechanism, or WAIVED with a ≥20-char reason if the fix is deferred to that submodule's FCC subplan — see the per-test-baseline backlog in `PLAN_BIG_PIVOT_FCC_MASTER.md`). Pre-commit Gate E (`npm run check:per-test-baseline`) blocks an unguarded save-spec; the glob WARN pass flags any save-capable spec missing from the registry. Cross-ref: LR-019, BUILDER HARD STOP #13 (atomic un-skip + harden), LR-067 (revert must verify persistence).
...
content-anchored reads) — **NO new runner**. **HARD STOP**: never author an SBC spec block without a
corresponding SBC section in the GIVER catalog (mirror of HARD STOP #11 for FCC). Surface TCs are
**ordinary 3-segment TCs** (`TC-<MOD>-<SUB>-NNN`) carrying a `**Surface_Family**: <family> (QUICK|DEEP)`
line — **no `-SBC-`/`-SBC-MAX-` ID infix** (a 4th segment fails `check-tc-parity` G6). `check:tc-parity`
already covers them; the Phase-3 post-write parity
gate is unchanged; **NO new parity script**. Per LR-065 + the Case-Generation Standard.
