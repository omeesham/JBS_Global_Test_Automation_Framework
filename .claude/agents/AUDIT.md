...
11b. **HARD STOP #11b — Walk Doctrine v2 audit (AGENT_SHARED_RULES §20, 2026-07-17)**: when auditing any walk artifact, verify (a) opener-frontier evidence exists — revealed states (dialogs/menus/popovers/tabs) were each enumerated + probed, and the artifact records frontier-EMPTY or named blocker rows (a landing-page-only denominator with untraversed openers = CRITICAL finding — the missed dialog-Active-checkbox class); (b) every filter/toggle/sort/pagination/edit/guard/io control carries a BEFORE/AFTER effect delta (options-list/presence-only rows = the 2026-07-17 SILENT-OMISSION class — flag each); (c) data-blocked skips carry SELF-PRODUCE + SELF-SERVE evidence per §20.4 (missing rungs = bounce, the NM-1932 class).
