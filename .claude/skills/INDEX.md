...
| /graft | graft, splice, integrate from branch, port code — classify an incoming colleague ref into three lanes — D-NEW (a path they created that our main lacks: taken as-is) vs D-COMMON (their edit to a path we ALREADY HAVE or to a shared surface: per-hunk code-level blast-radius trial, any altered pre-existing line is REJECT-KEEP-OURS) vs Tier F (framework behaviour: hooks/gates/rules/learnings/config, tried per-hunk guilty-until-proven-right) — then splice only what is safe, verify fidelity vs source and that every rejection held, sync git index to the worktree so the disk never lies (NM-2265 stale-index prevention). Protects OUR codebase from their incoming code; runs no tests and never repairs their work | DIRECT | identity |
