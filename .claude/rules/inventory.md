...
**Proof-of-work trail (REUSES the existing artifact — no new types):** the per-field tier + raw evidence + Opus verdict live in the existing `clients/${ACTIVE_CLIENT}/specs_planning/_internal/walk-evidence-<module>-<DATE>.md`, which gains a per-field row — *worker tier used · raw evidence (DOM value / aria-invalid / inline-error / network 2xx / post-reload value) · Opus verdict (accepted / re-done by tier-N / why)*. No `walk-probes/` directory and no `walk-judgment-*` file — those were over-engineering and must NOT reappear.
