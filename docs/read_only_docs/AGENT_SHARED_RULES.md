...
| `clients/${ACTIVE_CLIENT}/specs_planning/_internal/field-case-generation.md` | READ | RW | READ | READ | READ | READ | RW |
| `clients/${ACTIVE_CLIENT}/specs_planning/_internal/field-case-catalogs/<module>-*.md` | READ | RW | READ | READ | READ | READ | RW |
| `clients/${ACTIVE_CLIENT}/reports/bugs/BUG-*.json` | READ | READ | CREATE | CREATE | READ | READ | RW |
| `scripts/**` | — | — | — | — | — | — | RW |
| `config/**` | — | — | — | — | — | — | RW |
| `.claude/skills/**` | — | — | — | — | — | — | RW |
| `plans/**` | — | — | — | — | — | — | RW |
| `.claude/agents/**` | — | — | — | — | — | — | RW (governance — direct edits; no SYNC-ONLY post-2026-04-27 Copilot eviction) |
...
**Bug Filing Artifact ownership** (added SP-CCE-04 2026-04-27 — Plan B Fix 4): `clients/${ACTIVE_CLIENT}/reports/bugs/BUG-*.json` files are pipeline-produced bug reports per LR-034 / HLR-016 / GEN-* bug-filing protocols. BUILDER and HEALER both **CREATE** (default-deny without this row blocked HEALER's HLR-016 bug-filing mandate). HUNTER / GIVER / WATCHDOG / GARDENER are READ-only consumers (verifiers, planners cite, auditors review). OWNER has full RW for governance edits. Filing discipline is governed by LR-034 Step 5 + LR-044 verification protocol regardless of author identity.

**Human-Controlled (NEVER modify)**: `.env*`, `playwright.config.*`, `package.json`, `tsconfig.json`, `.ci/*`

**Governance-owned (direct OWNER edits)**: `.claude/agents/*.md` files receive HARD STOPS / scope-clause / Modes additions via direct edits authorized under OWNER identity. There is no SYNC-ONLY enforcement — `npm run sync:mistakes` is a no-op for `.claude/agents/` post-2026-04-27 Copilot eviction (the underlying `scripts/sync-agent-mistakes.ts` retained pending separate decommission decision; AGENT_CONDENSED + AGENT_COMPACT_FORMAT envelopes are empty for these paths). Mistake graduation from `agent-mistakes.md` to agent rules is a manual OWNER process per ALL-077.
...
| ALL-034 | `clients/${ACTIVE_CLIENT}/reports/bugs/` directory = structured bug report storage. Each BUG-{MOD}-{NNN}.json has evidence, TC source, MCP verification. | Bug tracking integrity |
...
### Universal Bug Detection Rules (ALL-042..045)
...
| ALL-045 | **Bugs are visual too, and suggestions count — bug detection is NOT just functional/network errors.** ANY UI/UX/layout/rendering defect a walker notices on ANY walk is reportable, even with no failing test and no documented requirement (the ALL-042/044 examples are all functional — that scoping is the gap). **Two tiers**: (1) **Bug** — a defect (broken/overflowing render, mis-layout, wrong behavior, accessibility break, focus-trap) = **HIGH prio** → confirm + file via LR-034 (or `BUG-CANDIDATE` if not yet MCP-confirmed); (2) **Suggestion / improvement / missing-feature idea** the agent believes could help = **LOW prio** → record only (triaged later, mostly deferred unless big). Both land in the walk-evidence `## Observations` section (Bugs / Suggestions buckets); **every walk records `## Observations`, or an explicit `none`** — silence is a violation, not "nothing found". A render-state defect is invisible to an `aria-invalid`-only / CLI-headless check (false-green, WCAG ARIA21) — the visual presentation must be seen + asserted, not inferred from the attribute. Folds in the discussion-item triage (`feedback_discussion_item_not_bug.md`) as the ambiguous-finding sub-type of the Suggestions bucket. | Visual/UX defect or improvement idea dropped because it wasn't a functional error / had no requirement, OR a walk closed with no `## Observations` record |
...
- `.claude/agents/AUDIT.md` HARD STOP 0a (agent-level block)
