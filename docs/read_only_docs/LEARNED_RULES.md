...
**Generated-file exemption (2026-07-07, FP fix v2)**: even within a single staged commit, one FP class survived staged-scoping — a row that legitimately lists `plans/INDEX.md`. The pre-commit hook itself regenerates and re-stages that file (`.githooks/pre-commit` §1: `plans-reindex.mjs` → `git add plans/INDEX.md`), bumping its mtime *forward* past the honest `When` of the very row that carries it → guaranteed backdating HALT for an honest row. Fix: `isGeneratedFile()` in `validate-activity-log.mjs` exempts commit-regenerated paths (currently only `plans/INDEX.md`) from the mtime comparison. These are machine-generated (LR-035, never hand-authored) so their mtime is meaningless as a "when was this work done" signal. The exemption is per-*file*, not per-row: a real authored file named alongside `INDEX.md` is still fully checked — anti-backdating is narrowed, not weakened (unit-proven: `mixed-still-fires-on-real`). Add a path to the exemption set ONLY if a hook both regenerates AND re-stages it inside the commit; a plain generated artifact that is not re-staged mid-commit does not qualify.
