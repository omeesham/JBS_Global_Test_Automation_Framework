...
const EXCLUDE = (p) =>
  ...
  /^\.claude\/skills\/graft\/classify-incoming\.sh$/.test(p) || // classifies files on INCOMING colleague branches, most of which still carry the pre-rename test_cases_xlsx/ directory — it must keep matching the old path or their work misclassifies
