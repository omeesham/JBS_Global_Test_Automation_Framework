...
async function initVault(): Promise<void> {
  ...
  console.log('[OK] Vault created successfully');
  console.log(`   Version: ${info.version}`);
  console.log(`   Created: ${info.createdAt}`);
  console.log('');
  console.log('[tip] Set VAULT_PASSPHRASE environment variable to avoid prompts:');
  console.log('   export VAULT_PASSPHRASE="your-passphrase"');
  console.log('');
  console.log('[WARN] IMPORTANT: Remember your passphrase! It cannot be recovered.');
}
...
async function setSecret(key: string, value: string): Promise<void> {
  ...
  console.log(`[OK] Secret "${key}" stored successfully`);
}
...
async function deleteSecret(key: string): Promise<void> {
  ...
  console.log(`[OK] Secret "${key}" deleted`);
}
...
async function rotateVault(): Promise<void> {
  ...
  if (newPassphrase.length < 8) {
    console.error('[ERR] New passphrase must be at least 8 characters');
    process.exit(1);
  }
  ...
  console.log('[OK] Vault passphrase rotated successfully');
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'set':
        if (args.length < 3) {
          console.error('[ERR] Usage: npm run vault:set KEY VALUE');
          process.exit(1);
        }
      ...
      case 'get':
        if (args.length < 2) {
          console.error('[ERR] Usage: npm run vault:get KEY');
          process.exit(1);
        }
      ...
      case 'delete':
        if (args.length < 2) {
          console.error('[ERR] Usage: npm run vault:delete KEY');
          process.exit(1);
        }
    ...
    console.error(`[ERR] Error: ${(error as Error).message}`);
    rl.close();
    process.exit(1);
  }
}
