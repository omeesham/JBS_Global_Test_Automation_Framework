...
**Headless degradation**: artifacts produced without Rovo carry `rovo_available: false`; the committed `jira-defect-crossref-*` file is consumed by headless `/chain` runs in lieu of a live Rovo call (per framework rule LR-063 headless caveat).
