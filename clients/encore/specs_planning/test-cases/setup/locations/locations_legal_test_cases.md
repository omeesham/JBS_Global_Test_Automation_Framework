...
## TC-LOC-LGL-019: No UI path to submit out-of-list SC value (negative enumeration + save-cycle) (FCC)
| Priority | Status | Type |
|----------|--------|------|
...
**Implementation rationale** (recorded 2026-05-27): Phase 3.0b probe was blocked at SSO redirect; engineering-knowledge fallback predicted Path C (Radix React state isolation). Phase 4 audit + spec run revealed **Path D** — DOM mutation of the Radix SC combobox node (with OR without synthetic events) crashes the Angular page (`Application error: a client-side exception has occurred`). The textContent-tamper vector is unsafe to automate. **Pivot**: implement the genuinely-uncovered mechanic ("server-side validation of dropdown values" / "no UI path to submit invalid values") via **negative listbox enumeration + full save-cycle** — proves (a) no UI affordance exposes the invalid value to the user, and (b) the legit save persists the correct value at the server boundary.

**Steps**:
1. **Negative enumeration** — open the SC listbox via `getServiceChargeOptions()`; assert (a) `LEGAL_INVALID_SC_VALUE = "INVALID_SC_DOM_TAMPER_VALUE"` is NOT among the 114 options, (b) `LEGAL_DEFAULTS.serviceChargeName` IS present (positive baseline check), (c) `LEGAL_ALT_SC` IS present (the value the save-cycle will use)
2. **Baseline** — `ensureDefaultState(LEGAL_DEFAULTS)` (LR-019)
3. **Act** — `selectServiceCharge(LEGAL_ALT_SC = "Administrative Fee")` through legit Radix listbox path
4. **expectBeforeSave** — Save now legitimately enabled via Radix→Angular `onValueChange`; SC value reads "Administrative Fee"
5. **saveAndConfirm** — click Save → Save Changes dialog → click Ok → save completes
6. **expectAfterSave** — Save button returns to disabled
7. **Reload** — `reloadAndNavigateToLegalTab()`
8. **expectAfterReload** — persisted value = "Administrative Fee" AND ≠ `LEGAL_INVALID_SC_VALUE` (negative end-to-end proof at server boundary)
9. **Cleanup** — `ensureDefaultState(LEGAL_DEFAULTS)` restores SC to "Resort Service Charge" + T&C to "LDW"

**Expected**: All assertions pass. The test proves (a) no legitimate UI surface exposes an out-of-list value, and (b) a normal save persists the correct legit value (never the sentinel) at the server boundary.

**Data**: office=1604, invalidSentinel="INVALID_SC_DOM_TAMPER_VALUE" (LEGAL_INVALID_SC_VALUE), legitValue="Administrative Fee" (LEGAL_ALT_SC), default="Resort Service Charge" (LEGAL_DEFAULTS.serviceChargeName)

**Cleanup**: `ensureDefaultState(LEGAL_DEFAULTS)` in the runner's `cleanup:` callback. Test is self-cleaning.

**Live finding** (recorded 2026-05-27): DOM tamper of the Radix SC combobox crashes the page even with text-only mutation (no synthetic events). This is a defensive characteristic of the app — no easy text-injection vector exists. Documented in field-case-catalog (legal-2026-05-27.md §TC-019 actual implementation outcome) as a parallel security observation.

**Future-flip path** (network-layer extension): if a future agent runs a seeded probe with `page.route()` interception of `/navigator/api/` save POSTs to inject `LEGAL_INVALID_SC_VALUE` into the request body and observes:
- 4xx response → server validates (defense in depth confirmed) — add assertion to TC-019, keep current assertions
- 2xx response → file BUG-LOC-LGL-001 (server lacks enum validation), append `test.fixme()` variant referencing the bug

…both extensions are additive to current assertions, not replacements.
