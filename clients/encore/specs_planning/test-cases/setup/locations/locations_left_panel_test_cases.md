...
- Form dirty (any change made) -- enabled, even with required field empty (LocalOfficeName error icon shows but Save still enables — live behaviour differs from documented `the invalid state` condition)
...
**Notes**: clearing LocalOfficeName shows error icon BUT Save enables (the clean state = false overrides). Differs from documented `the invalid state disables Save` condition. This discrepancy should be noted as a observation.
**Automatable**: Yes
...
**Expected**: Servicing Branch Office has 215 options; GL Servicing Division must be ≥1 (selecting a value satisfies validation)
...
**Notes**: Live Date confirmed as "May 8th, 2007" for office 1604. Displayed as "Month Day, Year" format.
**Automatable**: Yes
...
**Notes**: Legal tab validation cross-tab interaction. A separate Legal test suite covers dedicated Legal testing. This TC verifies only the cross-tab Save button behavior.
