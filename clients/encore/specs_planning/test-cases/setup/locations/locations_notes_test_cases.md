...
**Module**: locations | **Total**: 63 (37 existing main+HIST + 26 FCC; this FCC test deferred excluded) | **Status**: Manual | **Updated**: 
...
**Data**: office=1604
**Notes**: CORRECTION from : Table starts EMPTY (no default row). No `<thead>` exists in interface.
...
**Notes**: CORRECTION from : Limit is SOFT. No `maxlength` on textarea. Paste can exceed 4000.
...
## TC-LOC-NTS-033: Sequential save — add second note in same session, both persist
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | Round-trip | Yes |
...
**Expected**: Both notes persist after two sequential saves with reload between saves (the the unsaved state requires reload — RCA : Save remains disabled after add+fill in post-save session, multiple runs)
**Data**: office=1604 | text0="Sequential A" (12) | text1="Sequential B" (12)
**Cleanup**: Delete all rows and save to restore empty state

---

## TC-LOC-NTS-034: Edit existing saved note — overwritten text persists
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | Round-trip | Yes |
...
**Expected**: In-place edit of saved note persists after second save + reload (the the unsaved state requires reload between saves — RCA : Save remains disabled after fill in post-save session, multiple runs)
...
## TC-LOC-NTS-036: Overage content persists — 4001 chars save+reload without truncation
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | Round-trip + BVA | Yes |
...
## TC-LOC-NTS-037: Delete row persists without explicit textarea clear
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
...
## HIST per-column tests (col 69)

These test cases assert the Location Management History "Notes" column (column 69, 0-indexed) byte-exact contents after saving a note. The aggregate column serializes the Notes FormArray rows per the encoding rule documented in [`catalogs/hist-root-map-location-management-notes.md`](./././catalogs/hist-root-map-location-management-notes.md) State-space coverage matrix and verified for 4 additional text classes by the inline discovery probe at [`reports/live-probe-notes-col69.md`](././././././reports/live-probe-notes-col69.md).

Encoding rule: column 69 equals the rows joined by " | " where each row serializes as "MM/DD/YYYY - <row.value>" (today's save date plus a literal " - " plus the textarea content). When the app keeps an empty placeholder row after saving (per BUG-LOC-NTS-003), the column ends with an additional " | MM/DD/YYYY -" segment. The companion spec at `` builds the expected value from the observed pre-save row values, so both single-row and placeholder-trailing forms are handled with strict equality.

## TC-LOC-NTS-028: HIST col 69 — empty to "hello" save round-trip
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip | Yes |

**Depends_On**: TC-LOC-NTS-001 (baseline empty state)
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. The page loads and the Notes area is visible. 2. Make sure no rows are saved (clear any existing rows and save back to empty using the clear-textarea-then-delete-then-save sequence). 3. Click "Add" to create a new empty note row. 4. Type the text "hello" into the new note row. 5. Click "Save" on the left panel. A confirmation dialog appears. 6. Click "Ok" on the dialog to confirm the save. The dialog closes and the save is persisted. 7. Click the "Location Management History" tab at the top of the page. The history table loads. 8. Sort the history table by "Modified On" in descending order. 9. Read the value in the "Notes" column on the newest row at the top of the table.
**Expected**: The "Notes" column on the newest row of the history table shows today's date followed by " - hello", for example "05/11/2026 - hello". When the app also kept an empty placeholder row after saving, the column instead shows two segments joined by a vertical bar: "05/11/2026 - hello | 05/11/2026 -". Both forms are accepted by the spec because the expected value is built from the rows actually present at save time.
**Data**: office=1604 | text="hello" | history_column="Notes" (column 69 of 87, 0-indexed)
**Cleanup**: Clear all notes and save back to empty before the next test.

---

## TC-LOC-NTS-029: HIST col 69 — long text 4000 chars at boundary
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip + BVA | Yes |

**Depends_On**: TC-LOC-NTS-028
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. 2. Make sure no rows are saved. 3. Click "Add" to create a new row. 4. Fill the new note row with exactly 4000 "A" characters. The character counter shows "4000/4000". 5. Click "Save" on the left panel. The confirmation dialog appears. 6. Click "Ok" on the dialog to confirm the save. 7. Click the "Location Management History" tab. 8. Sort by "Modified On" descending. 9. Read the "Notes" column on the newest top row.
**Expected**: The "Notes" column on the newest row shows today's date followed by " - " followed by the 4000-character string of "A". The content is saved without truncation. When the app also kept an empty placeholder row, the column ends with an additional " | MM/DD/YYYY -" segment.
**Data**: office=1604 | text="A" repeated 4000 times | history_column="Notes"
**Cleanup**: Clear all notes and save back to empty before the next test.

---

## TC-LOC-NTS-030: HIST col 69 — special characters preserved verbatim
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip + Security | Yes |

**Depends_On**: TC-LOC-NTS-028
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. 2. Make sure no rows are saved. 3. Click "Add" to create a new row. 4. Fill the new note row with the literal string containing angle-bracket tags, an ampersand, double quotes, an apostrophe, and backticks. The exact payload is: angle-script-tag, plain text "alert(1)", closing angle-script-tag, space, ampersand, space, quoted "quotes", space, apostrophed "apos", space, backticked "tick", space, then angle-div-tag. 5. Click "Save" on the left panel. The confirmation dialog appears. 6. Click "Ok" on the dialog to confirm the save. 7. Click the "Location Management History" tab. 8. Sort by "Modified On" descending. 9. Read the "Notes" column on the newest top row.
**Expected**: The "Notes" column shows today's date followed by " - " followed by the exact payload bytes. Angle brackets, ampersands, quotes, apostrophes, and backticks are all preserved as plain text — there is no HTML escape and no character replacement. When the app also kept an empty placeholder row, the column ends with an additional " | MM/DD/YYYY -" segment.
**Data**: office=1604 | text=angle-script-alert-payload-with-quotes-and-backticks (56 chars) | history_column="Notes"
**Cleanup**: Clear all notes and save back to empty before the next test.

---

## TC-LOC-NTS-031: HIST col 69 — newlines preserved as literal characters
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip | Yes |

**Depends_On**: TC-LOC-NTS-028
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. 2. Make sure no rows are saved. 3. Click "Add" to create a new row. 4. Fill the new note row with three lines of content separated by newline characters: the first line "line1", a newline, the second line "line2", a newline, and the third line "line3". The total length is 17 characters. 5. Click "Save" on the left panel. The confirmation dialog appears. 6. Click "Ok" on the dialog to confirm the save. 7. Click the "Location Management History" tab. 8. Sort by "Modified On" descending. 9. Read the "Notes" column on the newest top row.
**Expected**: The "Notes" column shows today's date followed by " - " followed by the three lines with the newline characters preserved verbatim. Newlines are not replaced with spaces, line-break markers, or pipe separators. When the app also kept an empty placeholder row, the column ends with an additional " | MM/DD/YYYY -" segment.
**Data**: office=1604 | text="line1" plus newline plus "line2" plus newline plus "line3" (17 chars total) | history_column="Notes"
**Cleanup**: Clear all notes and save back to empty before the next test.

---

## TC-LOC-NTS-032: HIST col 69 — unicode characters preserved (Latin diacritic, CJK, dingbat)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip + Boundary | Yes |

**Depends_On**: TC-LOC-NTS-028
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. 2. Make sure no rows are saved. 3. Click "Add" to create a new row. 4. Fill the new note row with a unicode string mixing Latin diacritic letters ("café résumé"), Chinese CJK characters, and a dingbat checkmark symbol. 5. Click "Save" on the left panel. The confirmation dialog appears. 6. Click "Ok" on the dialog to confirm the save. 7. Click the "Location Management History" tab. 8. Sort by "Modified On" descending. 9. Read the "Notes" column on the newest top row.
**Expected**: The "Notes" column shows today's date followed by " - " followed by the unicode string with all characters preserved exactly. Accented Latin letters, CJK characters, and the dingbat symbol all round-trip without normalization or substitution. When the app also kept an empty placeholder row, the column ends with an additional " | MM/DD/YYYY -" segment.
**Data**: office=1604 | text="café résumé" plus two Chinese characters plus a dingbat checkmark | history_column="Notes"
**Cleanup**: Clear all notes and save back to empty before the next test. This is the last HIST per-column test, so leave Office 1604 at the catalog baseline empty state.

---

## TC-LOC-NTS-038: HIST col 69 — sequential save (A then B) produces 2 distinct history rows
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| Medium | Manual | Round-trip + State | Yes |

**Spawned_From**: PLAN_FCC_NOTES_COMPLETION_2026-05-21 Phase 1.6 Path A (this FCC test Notes-side rename; HIST counterpart). The Notes-spec this FCC test was realigned to assert value-persistence only; this HIST-side TC owns the "2 distinct history rows, one per save, not merged" assertion that this FCC test no longer makes.
**Depends_On**: TC-LOC-NTS-028
**Steps**: 1. Open the "Notes" sub-tab on Location Settings for office 1604. 2. Make sure no rows are saved (clear any existing rows and save back to empty using the clear-textarea-then-delete-then-save sequence). 3. Click "Add" to create a new empty note row. 4. Type the text "HIST seq A" into the new note row. 5. Click "Save" on the left panel. A confirmation dialog appears. 6. Click "Ok" on the dialog to confirm Save A is persisted. 7. Reload the page and return to the "Notes" sub-tab. 8. Clear the row 0 textarea (set its value to empty) and type "HIST seq B" into the same row. 9. Click "Save" on the left panel. The confirmation dialog appears. 10. Click "Ok" on the dialog to confirm Save B is persisted. 11. Click the "Location Management History" tab at the top of the page. 12. Sort the history table by "Modified On" in descending order. 13. Read the "Notes" column on every row whose "Modified On" timestamp is after the start of step 3 (the read window).
**Expected**: Two distinct history rows are present in the read window — one whose "Notes" column equals today's date followed by " - HIST seq A" (with an optional trailing " | MM/DD/YYYY -" placeholder segment), and another whose "Notes" column equals today's date followed by " - HIST seq B" (with the same optional trailing segment). The two rows have different "Modified On" timestamps and the row for "HIST seq B" is more recent than the row for "HIST seq A". The two saves did NOT merge into a single history row.
**Data**: office=1604 | textA="HIST seq A" (NOTE_SEQUENTIAL_HIST_A) | textB="HIST seq B" (NOTE_SEQUENTIAL_HIST_B) | history_column="Notes" (column 69 of 87, 0-indexed) | timestamp_column="Modified On"
**Cleanup**: Clear all notes and save back to empty so Office 1604 is restored to the catalog baseline empty state.

---

## Notes Field Coverage — TC-LOC-NTS-039.061 + collision-resolution TC-LOC-NTS-062/063/064
...
**Numbering gaps**: Original FCC numbering had gaps at this FCC test/004/005/011/030/031 (dropped/deferred at SP-NOTES-FCC-PILOT time); the post-rename canonical NTS-039.058 + NTS-060 + NTS-061 inherits those gaps (NTS-059 already used for the Sequential-save persist-value test — `TC-LOC-NTS-038` at line 575 owns the HIST-counterpart assertion). NTS-062/063/064 are the three collision-resolution IDs allocated by SUBPLAN_XLSX_PREP_01 for what were this FCC test/009/012.

---

## TC-LOC-NTS-039: 1-char persist (BVA min)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-040: 3999-char persist (BVA -1)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-041: Whitespace-only " " persist
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-042: Leading whitespace " hello" persist (not trimmed)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-043: Trailing whitespace "hello " persist (not trimmed)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-063: Tab character "a\tb" persist
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-046: Edit partial-replace (slice middle)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-047: Edit clear-to-empty (fill empty — row stays with empty value)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-048: 2-row positive (smallest multi-row save+reload)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-049: 5-row positive (smoke at moderate count)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-050: Mixed-content (row 0 = 1-char, row 1 = 4000-char) save+reload
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-051: Edit row 1 of 2 — row 0 value unchanged after save+reload
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-052: Delete first of 2 rows → row 1 becomes sole remaining row
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-053: Delete last of 3 rows → rows 0 and 1 remain
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-062: Delete one of one (single row) → empty state persists
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-054: Save → Cancel → edit → Save → Ok → final value persists (re-save flow)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-055: Save → dialog opens → reload page mid-dialog → no persist, no error
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-056: Save → Escape key on dialog → dialog closes, dirty preserved, no persist
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-057: Save → click outside dialog → observe whether dialog dismisses or stays
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-058: Idempotent save (save → state pristine → save attempt → button stays disabled)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
**Expected**: A second save attempt on a pristine form is a no-op; the Save button remains disabled; no network save request is dispatched.
**Data**: office=1604 | NOTE_IDEMPOTENT="Idempotent test"
**Cleanup**: ensure the table is empty

---

 One MD section dropped by SUBPLAN_XLSX_PREP_01 (former 28-suffix sequential-save-HIST test): true content-duplicate of TC-LOC-NTS-038 (Notes-saved-shows-in-HIST cross-test; canonical entry at line 575 + HIST spec at + CSV row at locations_notes_test_cases.csv:39). Per fcc-truth-investigation.md Q3 + content-dedupe-audit §3.1. Per Rutvik directive : notes stay in Notes module; History counterpart spec lives at specs/locations/history/. The underlying test survives under TC-LOC-NTS-038; only the duplicate MD section is removed. 

## TC-LOC-NTS-059: Sequential save persists most recent value (HIST verification deferred to NTS-038)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Automated | FCC | Yes |

**Depends_On**: none
**Spawned_From**: SUBPLAN_XLSX_PREP_01 backfill — spec at and CSV row at locations_notes_test_cases.csv:60 pre-existed without an MD entry from SP-NOTES-FCC-PILOT closure; backfilled here to satisfy Spec ⊂ MD ⊂ CSV parity. Splits responsibility with TC-LOC-NTS-038 (HIST 2-row distinctness assertion stays on the HIST spec side).
...
**Expected**: After two sequential saves of different values, row 0 holds the most recent value (B); the first value (A) is no longer present on the Notes form. HIST 2-row distinctness assertion is covered by TC-LOC-NTS-038 in the History spec .
**Data**: office=1604 | NOTE_SEQUENTIAL_HIST_A="HIST seq A" | NOTE_SEQUENTIAL_HIST_B="HIST seq B"
**Cleanup**: ensure the table is empty

---

## TC-LOC-NTS-060: Save Notes → switch to Currency tab → verify Currency NOT dirty (cross-tab isolation)
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
...
## TC-LOC-NTS-061: Post-save row count = N+1 placeholder check — assert content of rows 0.N-1 NOT total count
| Priority | Status | Type | Automatable |
|----------|--------|------|-------------|
| High | Manual | FCC | Yes |
