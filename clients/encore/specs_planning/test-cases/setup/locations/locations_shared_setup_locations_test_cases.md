...
| Priority | Status | Type |
|----------|--------|------|
| High | FIXME (app bug — BUG-LOC-SHR-001) | Functional |
...
**Known issue**: Blocked by app bug — Miami-region offices structurally excluded from /api/location/location-lookup visibility filter for office 1604; search "1233" returns a phantom row with empty localOffice cell. Pending Encore fix. See `reports/bugs/BUG-LOC-SHR-001.json` verificationLog.
...
## TC-LOC-SSL-028: Top-tab switch with dirty form shows Unsaved Changes dialog; Stay preserves state
| Priority | Status | Type |
|----------|--------|------|
| Medium | Automated | Functional |
...
## TC-LOC-SSL-030: Add three non-Miami rows (Chicago/Boston/Marriott) + save + reload → all three persist
| Priority | Status | Type |
|----------|--------|------|
...
5. Cleanup (LR-024 net-zero): delete all non-self rows + Save until only self-row remains; reload + verify count = 1
...
**Known issue**: Blocked by app bug — random per-row Delete button becomes non-clickable after add + save + reload; cleanup loop spins forever clicking the dead button. Pending Encore fix. Full ceiling characterization (proven up to 44 rows on throwaway office 1605) is out of CI scope — net-zero discipline on baseline office 1604 keeps mutation minimal.
...
| G19 | Checkbox save-cycle: toggle-then-revert disables Save | checkbox | (b) covered | TC-LOC-SSL-007 (line 193, fixme'd by LR-026/BUG-LOC-SHR-001 evidence vehicle) |
...
| G42 | Already-added location excluded from dialog | dialog | (b) covered | TC-LOC-SSL-024 (line 553, fixme'd by BUG-LOC-SHR-001) |
...
## TC-LOC-SSL-033: Search 1-char filter shows ≥1 result (BVA min)
| Priority | Status | Type |
|----------|--------|------|
...
## TC-LOC-SSL-034: Search very-long string (200+ chars) does not crash dialog (BVA max)
| Priority | Status | Type |
|----------|--------|------|
| Medium | Pending automation | Functional / α |
...
## TC-LOC-SSL-035: Search clear-input restores full row count (BVA empty after non-empty)
| Priority | Status | Type |
|----------|--------|------|
| Medium | Pending automation | Functional / α |
...
## TC-LOC-SSL-039: Search edit-cycle: type → clear → re-type swaps results
| Priority | Status | Type |
|----------|--------|------|
...
## TC-LOC-SSL-031: Multi-row delete-MIDDLE + save + reload (3 → 2 with middle gone) [FIXME]
| Priority | Status | Type |
|----------|--------|------|
| High | FIXME (BUG-LOC-SHR-001 random per-row Delete-after-reload) | Persistence / δ |
...
**Known issue**: BUG-LOC-SHR-001 (random per-row Delete becomes non-clickable after reload); cleanup loop may hang. Same fixme pattern as TC-030.

---

## TC-LOC-SSL-041: Multi-row delete-ALL non-self + save + reload (2 → 0 non-self)
| Priority | Status | Type |
|----------|--------|------|
...
## TC-LOC-SSL-042: Multi-row cross-row edit-preserve (toggle non-self SI → self SI unchanged across save)
| Priority | Status | Type |
|----------|--------|------|
...
## TC-LOC-SSL-032: Multi-row N=5 boundary push — add 5 non-Miami rows + save + reload [FIXME]
| Priority | Status | Type |
|----------|--------|------|
| Medium | FIXME (BUG-LOC-SHR-001 random per-row Delete after reload during cleanup) | Persistence / ε |
...
**Known issue**: Same BUG-LOC-SHR-001 random-Delete-non-clickable bug as TC-030 / this TC. Cleanup at end may hang.

---

## TC-LOC-SSL-043: Checkbox cross-row independence (pre-save) — toggle non-self SI does not flip self SI in-page
| Priority | Status | Type |
|----------|--------|------|
...
## TC-LOC-SSL-044: Checkbox full round-trip — self SI ON → save → reload → OFF → save → reload (round-trip persistence)
| Priority | Status | Type |
|----------|--------|------|
...
3. click Save and confirm the dialog → `click Save and confirm the dialog` (final save in the round-trip; saveAndVerifyCase only invokes once — intermediate save handled in act)
...
**Note**: this TC deviates from one-act-per-test purity for the round-trip motion — pattern parallels the equivalent Notes round-trip TC (edit-twice round-trip). The `act` callback performs both legs of the round-trip; saveAndVerifyCase's `click Save and confirm the dialog` + `reload` + `expectAfterReload` cover only the FINAL leg. Tier-1 (UI cache invalidation) verified via final reload; intermediate state captured inline in `act` via inline expects.
...
**Fixme count**: 2 (SSL-031 + SSL-032, both blocked by BUG-LOC-SHR-001 random-Delete-non-clickable bug — same app issue as existing TC-030).
