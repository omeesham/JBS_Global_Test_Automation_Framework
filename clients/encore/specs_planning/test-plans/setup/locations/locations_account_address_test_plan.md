...
**Module**: locations | **Updated**: 2026-06-11
**Test Cases**: specs_planning/test-cases/setup/locations/locations_account_address_test_cases.md
...
| TC-LOC-ACC-029 | tabAccountAndAddress, txtAccPhone2, btnSaveAccountAddress, dlgSaveChanges, btnSaveChangesConfirm |
| TC-LOC-ACC-030 | btnAccName, dlgAccountList, txtAccListAccountNumber, btnAccListSearch, tblAccListResults, btnAccListCancel |
| TC-LOC-ACC-031 | btnAccVenueAddress, dlgSelectAddress, txtAddrSearch, tblAddrResults, btnAddrCancel |
| TC-LOC-ACC-032 | btnAccMasterAddress, dlgSelectAddress, tblAddrResults, btnAddrSelect, btnSaveAccountAddress (Master display dd readers + Venue-unchanged check) |
| TC-LOC-ACC-033 | btnAccMasterAddress, dlgSelectAddress, tblAddrResults, btnAddrSelect, btnSaveAccountAddress, dlgSaveChanges, btnSaveChangesConfirm (saveAndVerifyCase persistence + restore-anchored) |
...
## Scenario: TC-LOC-ACC-029 - Phone 2 cleared value persists empty after reload
1. Step: baseline ensureDefaultState() then seed Phone 2 = TEST_PHONE2_VALUE and save, expected: Phone 2 has a value
2. Step: fillPhone2(''), expected: Phone 2 cleared, Save enables
3. Step: clickSave() (confirm "Ok"), expected: Save disables
4. Step: reloadAndNavigate(OFFICE_NO), expected: page reloads to A&A tab
5. Step: expect getPhone2Value() == '', expected: cleared value persisted
6. Step: cleanup ensureDefaultState(), expected: Phone 2 restored to empty baseline (STRICT-LINE-F)

---

## Scenario: TC-LOC-ACC-030 - Account List Account Number filter returns matching account
1. Step: openAccountListDialog(), expected: dialog visible
2. Step: searchAccountByNumber('AC000107'), expected: results filtered (poll ≤20s — server search)
3. Step: expect.poll accountListResultsContain('Parker Palm Springs'), expected: true
4. Step: cancelAccountListDialog(), expected: dialog closes

---

## Scenario: TC-LOC-ACC-031 - Address dialog search filter then clear restores full set
1. Step: openVenueAddressDialog(), expected: dialog visible with 7 rows (ADDRESS_SEARCH.totalRows)
2. Step: searchAddress(ADDRESS_SEARCH.filterTerm), expected: rows reduce (client-side filter)
3. Step: searchAddress(''), expected: filter cleared
4. Step: expect.poll getAddressRowCount() == ADDRESS_SEARCH.totalRows, expected: 7 (full set restored)
5. Step: cancelAddressDialog(), expected: dialog closes

---

## Scenario: TC-LOC-ACC-032 - Master Bill To selection updates Master display + leaves Venue unchanged + enables Save
1. Step: expect.poll getMasterCityText() == MASTER_BILL_TO_ORIGINAL.city, expected: 'WEST HOLLYWOOD'
2. Step: openMasterAddressDialog(), selectAddressRow(ALT_ADDRESS.address1), expected: dialog closes
3. Step: expect.poll getMasterCityText() == ALT_ADDRESS.city, expected: 'PALM SPRINGS'
4. Step: expect getVenueCityText() == ORIGINAL_ADDRESS.city, expected: 'WEST HOLLYWOOD' (Venue unchanged — Master isolated)
5. Step: expect.poll isSaveEnabled() == true, expected: form dirty
6. Step: reloadAndNavigate(OFFICE_NO), expected: Master display reverts to original (no persist without save)
7. Step: expect.poll getMasterCityText() == MASTER_BILL_TO_ORIGINAL.city, expected: 'WEST HOLLYWOOD'

---

## Scenario: TC-LOC-ACC-033 - Master Bill To selection persists through save+reload (restore anchored original)
1. Step: baseline — Master display == MASTER_BILL_TO_ORIGINAL (anchored before change)
2. Step: openMasterAddressDialog(), selectAddressRow(ALT_ADDRESS.address1), expected: Master city → PALM SPRINGS
3. Step: saveAndConfirm() (Ok), reloadAndNavigate(), expected: getMasterCityText() == 'PALM SPRINGS' (persists — diverges from Venue TC-027)
4. Step: cleanup/finally — openMasterAddressDialog(), selectAddressRow(MASTER_BILL_TO_ORIGINAL.address1), saveAndConfirm(), reload, expected: all 5 Master values back to anchor
5. Step: via saveAndVerifyCase runner (LR-019 compile-required baseline); no env leak (post-run billToAddress = 8899 Beverly, id 8ad746d8)
...
Authoritative current case list (33 cases). Scenario prose above may lag; this index is mechanically regenerated.
...
- TC-LOC-ACC-021 — Verify the Phone 1 value persists after save and reload
