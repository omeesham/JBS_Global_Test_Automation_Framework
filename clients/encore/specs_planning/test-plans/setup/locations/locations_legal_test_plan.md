...
**Module**: locations
**Test Cases**: specs_planning/test-cases/setup/locations/locations_legal_test_cases.md
**Updated**: 2026-05-27 (FCC Scenario for TC-LOC-LGL-019 added per SUBPLAN_LEGAL_FCC)
...
## Scenario: TC-LOC-LGL-019 - No UI path to submit out-of-list SC value (negative enumeration + save-cycle) (FCC)
**FCC group**: negative (no UI path to invalid value) + runner-integration smoke; runner = `saveAndVerifyCase()` from `clients/encore/src/core/field-case-runner.ts`. Pivoted from DOM-tamper to listbox-enumeration after Phase 4 audit (2026-05-27) observed Path D — DOM mutation of the Radix SC combobox crashes the Angular page.

1. Step: Navigate fresh, click tab[tabLegal], expected: Legal tab loads
2. Step (negative enumeration): Call getServiceChargeOptions() to read all 114 options. Assert (a) options DO NOT contain "INVALID_SC_DOM_TAMPER_VALUE" (no UI path to submit invalid value), (b) options DO contain "Resort Service Charge" (positive baseline), (c) options DO contain "Administrative Fee" (the save-cycle value)
3. Step (baseline): ensureDefaultState({SC="Resort Service Charge", T&C="LDW"}), expected: defaults loaded, button[btnSaveLegal] disabled
4. Step (act): selectServiceCharge("Administrative Fee") via legit Radix listbox path, expected: SC display = "Administrative Fee"
5. Step (expectBeforeSave): Verify button[btnSaveLegal] enabled, getServiceChargeValue() === "Administrative Fee"
6. Step (saveAndConfirm): Click button[btnSaveLegal] → alertdialog[dlgSaveChanges] appears → Click button[btnSaveChangesConfirm] → save executes
7. Step (expectAfterSave): Verify button[btnSaveLegal] returns to disabled
8. Step (reload): reloadAndNavigateToLegalTab()
9. Step (expectAfterReload): Verify (a) getServiceChargeValue() === "Administrative Fee" (legit save persisted at server), (b) getServiceChargeValue() ≠ "INVALID_SC_DOM_TAMPER_VALUE" (negative-case proof — invalid value never reached server)
10. Step (cleanup): ensureDefaultState({SC="Resort Service Charge", T&C="LDW"}) restores

**Disposition** (resolved at BUILDER time 2026-05-27, post-Phase-4-audit): Test runs as a green `test()` block. Live run 2026-05-27 11:11: PASS (21.0s). Future seeded probe can EXTEND (not replace) with Path A (network-layer body-injection → 4xx) or Path B (network-layer body-injection → 2xx + BUG filing) assertions.
...
Authoritative current case list (19 cases). Scenario prose above may lag; this index is mechanically regenerated.
