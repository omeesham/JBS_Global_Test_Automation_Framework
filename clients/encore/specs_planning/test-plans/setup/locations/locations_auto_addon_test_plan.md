...
**Test Cases**: specs_planning/test-cases/setup/locations/locations_auto_addon_test_cases.md
...
| fieldCountReconciliation | PASS — 5 items for office 1604 (re-confirmed live 2026-06-11): Encore Music, Wireless Presenter, Express Content Design Session, Wordly, Labor. Item list is country-scoped (`auto-addon-types?countryId=1`), constant within a country. |
| tcPlanSync | PASS — All 20 scenarios match TC titles and steps 1:1 (re-synced 2026-06-11; TC-016 title corrected to country-scoped) |
...
## Scenario: TC-LOC-AAO-016 - Item List Is Country-Scoped (Same 5 Across Locations)
1. Step: Navigate to office 1604; tabAutoAddon.click(); count chkAutoAddonAll, expected: 5 items
2. Step: Observe the item-list load call, expected: `GET /navigator/api/core/auto-addon-types?countryId=1` (country-scoped, not location-scoped)
3. Step: Verify item list does NOT mutate per location, expected: same 5 items for all US locations; nav2 baseline shows the same 5
NOTE: Status: Manual — premise corrected 2026-06-11 (was "location-specific"). Country-variation case not reachable on US office 1604; NM-1462/64/65 = LR-040(c) discussion item.
...
Authoritative current case list (20 cases). Scenario prose above may lag; this index is mechanically regenerated.
...
- TC-LOC-AAO-016 — Item List Is Country-Scoped (Same 5 Across Locations)
