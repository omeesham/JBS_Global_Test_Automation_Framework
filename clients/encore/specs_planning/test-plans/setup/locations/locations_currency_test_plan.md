...
**Module**: locations
**Test Cases**: specs_planning/test-cases/setup/locations/locations_currency_test_cases.md
...
## Scenario: TC-LOC-CUR-028 - Reverting a currency selection re-disables Save (no net change)
1. Step: Start from the enforced default state, expected: USD selected+default, CAD/MXN unselected, Save disabled
2. Step: Click CAD row checkbox[Selected], expected: checked; button[Save] becomes enabled
3. Step: Uncheck CAD row checkbox[Selected] (revert to saved state), expected: unchecked
4. Step: Verify button[Save], expected: returns to disabled (form detects no net change — smart-diff dirty tracking)
