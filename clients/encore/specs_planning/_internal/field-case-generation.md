...
| 5 | guard | Dirty-state prompt appears AND both prompt actions (Stay / Leave) honored. Navigate-away is mandatory, not deferred to DEEP. | `PROBED` with `effectObserved:boolean`. | No RCA row bears on this class. §5 class 5 mandates navigate-away unconditionally, filling the §3 QUICK gap (§3 persistence QUICK covers sort/page-size survival only; dirty-state guard mandated at §3 DEEP L2 only). The dirty-state taxonomy hole appears as matrix row 4 but is never labeled in RCA prose. |
