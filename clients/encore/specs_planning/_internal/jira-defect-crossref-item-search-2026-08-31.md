...
| OR-4 | "for barcode verification I will provide few barcode numbers; it should filter those products" | **CLOSED 2026-09-01** — owner supplied 12 barcodes (read from asset tables in the owner's screenshot, which showed them as office 1121 assets — screenshot provenance, not a live read; all 12 resolve on the 1101 Products page, which IS a live read). All 12 verified live: each returns exactly one product; 5 distinct products across the set. Consumed by TC-ISR-PRS-022…030 (`SUBPLAN_ITEM_SEARCH_BARCODE_COVERAGE.md`). NM-1494's 42-char ceiling confirmed enforced; its charset is NOT policed on entry. |
