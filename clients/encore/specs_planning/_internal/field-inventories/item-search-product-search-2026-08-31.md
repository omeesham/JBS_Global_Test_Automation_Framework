...
| Barcode | `e2e-barcode-input` | Plain text | empty (after Reset) | none observed — alphanumeric accepted | enabled | filters to products under the entered barcode (owner-provided numbers pending for positive data) | Garbage value → "0 products found" + "No results" empty state. `affordance: none`. |
...
| Prep Date Time | `(none) — first "Open popover" button in the dates row` | Date/offset (calendar popover + time spinner) | today 12:00 AM | field-level only (owner ruling: date behavior not functional yet) | enabled | both dates ride every search request (availability enrichment) — excluded from assertions per owner ruling | `affordance: popover → calendar with time spinner`. |
| Return Date Time | `(none) — second "Open popover" button in the dates row` | Date/offset (calendar popover + time spinner) | today 11:59 PM | field-level only (same ruling) | enabled | same | `affordance: popover → calendar with time spinner`. |
...
- Every cell is a truncation-conditional tooltip trigger (tooltip shows full text only when truncated — positive + negative proven).
...
|---|---|---|---|
| `struct:button\|trigger-button\|div/div/skip/div/div/div` | button | 2026-08-31 | out-of-scope: app-shell sidebar toggle — page chrome outside this module's behavior denominator |
| `struct:button\|More information\|skip/div/div/div/div/div` | button | 2026-08-31 | covered-by-TC: TC-ISR-PRS-005 |
| `testid:e2e-search-section-wrapper` | div | 2026-08-31 | out-of-scope: non-interactive structural wrapper carrying no behavior of its own |
| `testid:e2e-search-form` | form | 2026-08-31 | out-of-scope: form container element — submission rides the Search button, not the form node |
| `testid:e2e-card` | div | 2026-08-31 | out-of-scope: non-interactive card wrapper carrying no behavior of its own |
| `testid:e2e-card-header` | div | 2026-08-31 | out-of-scope: non-interactive card header wrapper carrying no behavior of its own |
| `testid:e2e-card-title` | div | 2026-08-31 | out-of-scope: non-interactive title element carrying no behavior of its own |
| `testid:e2e-popover-trigger` | button | 2026-08-31 | covered-by-TC: TC-ISR-PRS-004 |
| `testid:e2e-card-content` | div | 2026-08-31 | out-of-scope: non-interactive content wrapper carrying no behavior of its own |
| `testid:e2e-toggle-group` | group | 2026-08-31 | out-of-scope: radio-group container — behavior carried by the toggle-keyword radio row below |
...
| `struct:th\|Category\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Sub Category\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Class\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Product Group\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Sub Class\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Item\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Product Code ID\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Description\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Available\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Owned\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Out of Service\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|In Sequence\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
...
| `struct:th\|Location Name\|div/div/div/table/thead/tr` | th | 2026-08-31 | out-of-scope: static header cell — interactive behavior lives on the header menu button row |
