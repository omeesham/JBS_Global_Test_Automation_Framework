...
<!-- Coverage manifest keys (LR-062)
Coverage_Ratio: 148/148 (100%)
...
# Field Inventory — Setup → Discount Optimization Settings
...
Two tabs on one route: **Discount Optimization** (the locations grid) and **Special Rate Exemptions by
Service Type**. The denominator below is machine-enumerated — it is not a human count.
...
Denominator: **148** machine-enumerated elements. Every row carries a disposition; there are no blanks.
...
| `id:radix-_r_2e_` | button | out-of-scope: global application shell and left-hand navigation, outside the Discount Optimization surface under test |
...
Search placeholders: tab 1 `Search by location number or location name`; tab 2 `Search by service type`.
...
- Tab 2 carries its own `Save` and `Cancel`. Its save cycle has not been exercised.
...
One item remains open and unproven, deliberately **not** filed: whether NM-3340's service-type filter
reached this environment. Tab 2 lists 29 types including several plainly non-equipment entries, which
matches the ticket's pre-fix Actual Result — but the Equipment-rollup taxonomy is client-owned and
...
Tab 2 is a 29-row grid with search: `result-fidelity` and `empty-vol` ride `TC-DOP-EXM-010`;
`sorting` is `out-of-scope: sorting=no sortable headers were enumerated on this tab`.
...
or the tab-2 service-type list (NM-3340 will change it). Otherwise this artifact is good for 14 days
from 2026-08-11.
...
Machine-derived keys cross-referenced against field inventory dispositions. Source: eports/walk-coverage/dop-tab1.json (148 entries).
