...
jira_tickets: [NM-2253, NM-2259, NM-2043, NM-1757, NM-1851, NM-2055, NM-2050, NM-1907, NM-2036]
...
| NM-1851 (QA Defect, Done) "Duplicate Product Group description is allowed during creation but blocks update operation later" — found by the list-page Jira pass on 2026-09-09, after this walk | Expected: duplicate-description validation at creation, "similar to duplicate name validation"; before the fix a duplicate description saved and then blocked every later update of that group. | A new name with an already-used description is rejected at Save with `Product group name '…' or group description '…' already exists.` (the same toast shape as the duplicate name). | fixed as stated → TC-ISR-APG-012 (the walk's "no stated requirement" discussion item is withdrawn — see Observations) |
...
| NM-1907 (QA Defect, Done, Edit page) "Save inconsistent when clearing required Name" | Save must stay disabled whenever Name is empty regardless of how it was cleared. | Edit page verified live 2026-09-10 (group id 4620, probe in .playwright-cli/apg-2026-09-10): select-all + Delete leaves Save disabled with the box marked invalid, and backspacing to empty does the same — fixed as stated. On the Add page both select-all+Delete and character-by-character backspace disable Save and mark the box invalid. | fixed — verified on Add; required TC |
...
- Rejection toasts **survive navigation but do expire, after about 10 s** — first read by the automation run of 2026-09-09 as "do not auto-hide"; corrected the same evening by the family run's TC-ISR-APG-012 flake: the two toasts TC-011 raised at 16:27:44.9Z and 16:27:46.3Z were gone by about 16:27:56Z and 16:28:02Z (the matching-toast count read 2 → 1 → 0 while TC-012 waited; trace + diagnostics of that attempt), so a count taken before a Save can shrink while the Save's own toast is awaited. The original reading follows (found by the automation run, not by the walk): the two duplicate-name toasts raised by TC-ISR-APG-011 were still on screen when TC-ISR-APG-012, in the same browser, had gone through Cancel, the list page and a fresh Add form — at least 12 s later. The failure screenshot showed the front toast reading `Product group name 'ZZ E2E Walk 2026-09-09 A' or group description 'duplicate name check 1788952487101' already exists.` (TC-011's values) on TC-012's form; the run log (`.playwright-cli/apg-2026-09-09/apg-spec-run1.txt` — the runner's console output, kept as `.txt` because `.log` files are ignored by git) keeps the strict-mode error listing both `<li data-sonner-toast data-type="error">` elements (`data-index` 0 and 1). The screenshot itself was not retained — the run-1 artifacts were cleared before run 2. The success toast, by contrast, hides in ~3 s. Consequence for the page object: a rejection read must wait for the toast the Save click itself raises (matching-toast count grows by one) — the first version matched "any toast with the text", resolved instantly on the leftover, and TC-011's second Save was never actually observed.
...
### Bugs / Defects
- **Ruled not a bug 2026-09-10 (owner) — recorded as a Jira contradiction; was BUG-CANDIDATE (requirement contradiction)** — post-save landing: live app → group list page; NM-2043 (2026-06-08) and NM-2055 (2026-06-01) state the group's details page, while NM-1757 comment 148496 (2026-05-19) states the landing search page. Two Jira statements point opposite ways over three weeks (PR 1813 list → "we changed to details"); the live app follows the older one. Re-verified 2026-09-10: five saves on 1101 (three automated, two probes in `.playwright-cli/apg-2026-09-10/`) all landed on the list page within about a second, with the success toast raised on the Add page and still showing on the list. The cases pin the observed list landing (TC-ISR-APG-004); the contradiction is left for the dev lead to settle in Jira.
- **Description uniqueness is a stated requirement after all** — a new name with an already-used description is rejected. The 2026-09-09 walk first recorded this as "enforced without a stated requirement" (discussion item); the list-page re-walk's Jira pass the same day found NM-1851 (QA Defect, Done): "Duplicate Product Group description is allowed during creation but blocks update operation later" — expected result: "duplicate description validation should be applied during Product Group creation itself … similar to duplicate name validation". The live rejection is that fix. Discussion item withdrawn; TC-ISR-APG-012 pins the fixed behaviour (correction recorded 2026-09-09).
- **Withdrawn 2026-09-09 (was BUG-CANDIDATE UX, owner ruling)** — a rejection toast does hide by itself after about 10 s (measured from the family run's TC-ISR-APG-012 flake, see the server-side rules above); an error toast that outlives a success toast and survives a route change for its lifetime is ordinary behaviour, not a defect. The original reading follows: a rejection toast never hides by itself and survives leaving the page: after a duplicate-name rejection the error is still showing on the group list and on a fresh, empty Add form until its × is clicked (≥ 12 s observed in the 2026-09-09 automation run — see Server-side rules), while the success toast hides in ~3 s. A user opening a new group sees "already exists" about a previous attempt. Not filed: a sticky error toast may be the intended notification style; the question is whether it should clear on navigation.
...
| `census:link\|Products\|breadcrumb (ref e181)` | link | 2026-09-09 / 01-resting.yml | out-of-scope: duplicate-of: `census:link\|Product Groups\|breadcrumb` — the same shell breadcrumb archetype and the same silent-discard exit; the nearer crumb is the one a user editing a group would reach for, and TC-ISR-APG-023 drives it |
| `census:link\|Product Groups\|breadcrumb (ref e185)` | link | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-023 |
| `census:textbox\|Search\|sub-class card (ref e206)` | textbox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-013, TC-ISR-APG-016 |
| `census:combobox\|sort order, visible trigger\|sub-class card (ref e207, testid select-currency)` | combobox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-015, TC-ISR-APG-016 |
...
| `census:checkbox\|Labor\|sub-class card (ref e211, testid e2e-checkbox)` | checkbox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-014, TC-ISR-APG-016 |
| `census:button\|Reset\|sub-class card (ref e213)` | button | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-016 |
| `census:button\|Collapse search panel / Expand search panel\|divider (ref e15008)` | button | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-019 (both labels of the one control) |
| `census:button\|Cancel\|form (ref e15012)` | button | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-003 |
| `census:button\|Save\|form (disabled at rest)` | button | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-001, TC-ISR-APG-004, TC-ISR-APG-009, TC-ISR-APG-011, TC-ISR-APG-012, TC-ISR-APG-020, TC-ISR-APG-021 |
| `census:textbox\|Enter Product Group Name\|form (ref e15022)` | textbox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-005, TC-ISR-APG-007, TC-ISR-APG-008, TC-ISR-APG-011, TC-ISR-APG-021 |
| `census:textbox\|Product Group Description\|form (ref e15030)` | textbox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-006, TC-ISR-APG-009, TC-ISR-APG-012 |
| `census:combobox\|Service Type, visible trigger\|form (ref e15040)` | combobox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-010, TC-ISR-APG-021 |
...
| `census:checkbox\|Active, visible\|form (ref e15046)` | checkbox | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-009, TC-ISR-APG-020 |
...
| `census:row\|catalog item [archetype×7,394]\|sub-class card (div[draggable=true])` | draggable row | 2026-09-09 / 01-resting.yml | covered-by-TC: TC-ISR-APG-017 (double-click add), TC-ISR-APG-018 (drag add), TC-ISR-APG-013 / TC-ISR-APG-014 / TC-ISR-APG-015 (filtered and sorted reads) |
| `census:option\|Service Type option [archetype×90]\|listbox (refs e15059…)` | option | 2026-09-09 / 02-service-type-open.yml | covered-by-TC: TC-ISR-APG-010 (all 90 asserted verbatim in order; first, middle and last selected), TC-ISR-APG-021 (last option saved for real) |
| `census:option\|Ascending / Descending [archetype×2]\|listbox (refs e15333, e15337)` | option | 2026-09-09 / 03-sort-order-open.yml | covered-by-TC: TC-ISR-APG-015 |
| `census:button\|added sub-class × remove\|form (ref e58470; no accessible name)` | button | 2026-09-09 / 05-item-added.yml | covered-by-TC: TC-ISR-APG-017 |
| `census:button\|toast dismiss ×\|notifications region (present only while a toast shows)` | button | 2026-09-09 / 14-duplicate-toast.png | out-of-scope: outside-module — the notification region's dismiss control is app-shell chrome present on every page; the toast TEXT this page raises is asserted by TC-ISR-APG-004, TC-ISR-APG-011 and TC-ISR-APG-012, and its persistence is recorded under Observations |
...
`behavior-cases: result-fidelity (TC-ISR-APG-013, TC-ISR-APG-014 — every filtered row contains the term / is a labor row), sorting (TC-ISR-APG-015 — first and last row swap between Ascending and Descending), empty-vol (TC-ISR-APG-013 — no-match → 0 rows, cleared → the full catalog count read live), combination (TC-ISR-APG-016 — search + Labor + sort reset together with an added item kept), render-state (TC-ISR-APG-017 — the added row renders with its remove control and the instruction returns on removal; TC-ISR-APG-019 — panel geometry), persistence (TC-ISR-APG-022, TC-ISR-APG-023 — nothing survives leaving the page; picker state is not stored across visits, Live-state caveat)`
