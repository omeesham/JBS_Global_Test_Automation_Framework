...
| NM-1618 | Default order is Name ascending (A → Z) | the first page ascends by code point (`<` sorts before `A`); the Name header carries the ascending arrow from the first render — at rest, after a search and after Reset to Default View (spot-check 2026-09-09 15:40Z, fresh context; an earlier draft of this row said "no arrow until a sort" — wrong, corrected before the specs were written) | fixed as stated → TC-029 |
...
Divergences found live that no ticket states are listed under Observations (three BUG-CANDIDATEs, one withdrawn reading).
