...
class AgentReporter implements Reporter {
  ...
  // Per-test retry tracking (Layer 2 — derived in-process from Playwright's TestResult events)
 ...
 for (const [testId, finalStatus] of this.finalStatusByTestId.entries()) {
   if (finalStatus === 'passed') {
    ...
    for (const [retryN, count] of this.perTestFailedOnRetry.entries()) {
      ...
      // approximate per-attempt ms: total duration at this retry index ÷ events at that index
    ...
    const perTestStats: PerLayerStats = {
      ...
      // failedAfterAllAttempts = first-try failures that were NOT recovered on any retry
      failedAfterAllAttempts: Math.max(0, this.perTestFailedFirstTry - perTestRecoveredTotal),
    };

    // Layer 1 (clickWithRetry) + any other file-driven layers
