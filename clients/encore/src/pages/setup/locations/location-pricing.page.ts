/**
 * @agent-doc
 * PURPOSE: Location Pricing Tab Page Object -- Corporate pricing toggle, primary pricing dropdowns,
 *          secondary pricing grid (cascading checkboxes, date fields), currency filter, and
 *          Pricing-specific Save button (Setup > Location > [Office] > Pricing tab).
 * OWNER: generator
 * IMPACT: medium -- Pricing tab tests depend on this; changes affect all pricing specs.
 * DEPENDS-ON: BasePage, LocationSettingsSelectors, DynamicSelectors, logger.ts, framework-contracts/index.ts
 * USED-BY: tests/specs/setup/locations/location-pricing.spec.ts, fixtures.ts
 * RULES: Never use raw page.* in specs. All selectors from src/selectors/index.ts.
 *        Dynamic grid selectors via DynamicSelectors (not getElement).
 *        Save button is disabled when no changes are pending; enabled after form edits.
 *        Cascading: Is Alternative -> Use Effective Date -> Start/End Date.
 * NOTE: Extends BasePage directly (not LocationFormHelpers) by design.
 *       Pricing uses DynamicSelectors for grid rows and has a dedicated Save button
 *       unlike other tabs. No spinbutton boundaries, no reload-verify-persistence orchestration.
 */
...
export class LocationPricingPage extends BasePage {
   ...
   * MCP-verified 2026-03-02: combobox opens a dialog[name="Popover Content"] containing
   ...
   * Delegates to BasePage.getComboboxOptions (ALL-020: shared Radix listbox pattern). */
  ...
  private async selectDateFromCalendar(
    ...
    // MCP-RCA 2026-03-02: The popover trigger is a <div role="button" aria-label="Open popover">.
    ...
    // MCP-RCA 2026-03-26: dispatchEvent('click') fires a raw Event that React/Radix
    ...
    while (currentLabel !== targetLabel && safety < 24) {
      ...
      // MCP-RCA 2026-03-26: dispatchEvent('click') fires a raw Event that React ignores.
