...
>
> **Normal operation**: agents do NOT freely edit this file — element discoveries go to `specs_planning/test-cases/*.md`, field-inventories, and walk-evidence. **Authorized exception**: a HUNTER recovery/refresh pass (per `R11`, REQUIREMENTS.md is HUNTER-writable) reconciles this doc against verified live reality. Last such refresh: **2026-06-25** (PLAN_ENCORE_AGENT_DOCS_RECOVER_REFRESH).
> **Live module index**: the per-surface findings registry with dated evidence is `.claude/context/navigation.md` §C — consult it for current module structure and behaviors; this file is the durable narrative, navigation.md §C is the freshest pointer.

**Target Application**: [Navigator Cloud E2E Environment](https://cloudapps-e2e.encoreglobal.com/navigator/) — see `clients/encore/CLAUDE.md` for base URL / office / auth single-source-of-truth.

---

## Authentication System

Navigator Cloud uses **Microsoft SSO (Single Sign-On)** (Azure AD / Entra ID).

> **Automation-user reality (verified 2026-06-25)**: the test account has **NO second factor configured** (`clients/encore/CLAUDE.md` → Automation User Provisioning step 2), so the live login flow is **Continue Now → email → password → "Stay signed in?" → redirect** with **no TOTP/OTC step** (`clients/encore/src/pages/auth/login.page.ts` has no OTC handling). The TOTP machinery below is a *defensive capability* in `credential-loader.ts` (reads `NAVIGATOR_MFA_SECRET` if present) for a future MFA-enabled account — it does NOT fire for the current automation user. Do not treat MFA as a required step.
...
### MFA/TOTP Details (defensive capability — NOT active for the current automation user)
- `credential-loader.ts` reads an optional `NAVIGATOR_MFA_SECRET` (base32 TOTP seed) if one is supplied — support exists for a future MFA-enabled account.
- The current automation user has no second factor, so no OTC/Verify step occurs in the live flow (`login.page.ts`).
- If a future account requires MFA, codes are 6-digit TOTP (30s validity); provide the seed via the `NAVIGATOR_MFA_SECRET` env var.
...
- `clients/encore/src/selectors/<module>/` (per-module TypeScript selectors; barrel re-export at `src/selectors/index.ts`) — NOT a single flat `index.ts`
- `specs_planning/test-cases/setup/<module>/*.md` (test case documentation)
- `specs_planning/test-plans/setup/<module>/*.md` (technical automation plans)
...
- `NAVIGATOR_MFA_SECRET` - optional base32 TOTP seed (unused by the current no-2FA automation user)

Credentials are loaded at runtime by `CredentialLoader` (`clients/encore/src/utils/credential-loader.ts`) from environment variables.
...
3. Element selectors are stored per-module in `clients/encore/src/selectors/<module>/`
4. Test cases are documented in `specs_planning/test-cases/setup/<module>/*.md`
5. Element discoveries go to test-case / field-inventory / walk-evidence files; this file is refreshed only by an authorized HUNTER recovery pass (see purpose note at top)
...
- **Save endpoint architecture (new site) — CORRECTED 2026-06-25 (was inverted; see LR-056)**: saves commit to a **backend API endpoint** under `/navigator/api/...` (verified in live page objects: `POST /navigator/api/location/update-properties`, `POST /navigator/api/location/pricing/save`, `pricebook/upsert-location-pricebook`). Tests asserting save-cycle behavior MUST filter network on the `/navigator/api/` endpoint — **NEVER** on the page-URL POST: the Next.js App-Router fires server-component-render POSTs to the page URL for 0–8s after `reload()`, and a page-URL filter captures those as **false-positive saves** (LR-056 / ALL-086; prior text here said the opposite and was wrong). For unattended persistence checks, reload + re-read is the load-bearing assertion, not a page-URL listener. Per BUG-LI-001 verification 2026-04-27 — on server failure (e.g. 503) the new-site UI renders no error toast or dialog (silent failure mode); tests should still assert an error feedback surface (toast / dialog / inline message) on save failure as part of the user-feedback contract.
...
| Pay To Address | **launcher** (disabled input + clickable label) | Encore | input disabled, but the **label is a launcher** — clicking it (React onClick; `dispatchEvent('click')` / `{force:true}`) opens the **"Pay To List"** dialog; selection **PERSISTS** (`financial.payToId`). NOT a plain disabled textbox (LR-057; navigation.md §C launcher-dialogs row) |
...
**Test Architecture**: Location Management History coverage lives in **`tests/locations/location-management-history.spec.ts`** (path corrected 2026-06-25 — the old `tests/specs/setup/locations/history/*.spec.ts` layout predates the 2026-06-05 POM restructure; `specs/` was removed and the per-module dir is flat). Per-column `describe()` organization per the root field's control type was the 2026-04-20 design intent — verify the current single-file structure against the live spec before relying on it.
...
*Location Settings field inventories last MCP-verified 2026-04-13 (location 1604, all 8 right-panel tabs + left panel + Management History + Local Office Settings). Individual tabs re-walked later — see navigation.md §C for per-tab dates (Pricing 2026-06-19, Account & Address 2026-05-29, Legal 2026-05-27, Auto Add-On 2026-06-11, Left Panel 2026-06-03). Doc structure/paths reconciled 2026-06-25.*
...
**Test Architecture**: Local Office Settings History coverage lives in **`tests/local-office/local-office-history.spec.ts`** (path corrected 2026-06-25 — the old `tests/specs/setup/local-office/` layout predates the 2026-06-05 POM restructure). Per-column `describe()` organization per the root field's control type was the 2026-04-20 design intent — verify the current single-file structure against the live spec before relying on it.
...
## Corporate Pricing Page (`/settings/corporate-pricing`)

> **Added 2026-06-25** to close a documentation gap — this module is fully automated but had zero coverage here. It is a **separate Setup page** from Location Settings, and is **React / Next.js** (NOT Angular — `waitForAngularStable` is a no-op). **Detailed, dated field inventories live in `.claude/context/navigation.md` §C** + `clients/encore/specs_planning/_internal/field-inventories/corporate-pricing-*.md`; this section is the orientation summary, not a re-transcription.

**Sub-surfaces (one spec each in `tests/corporate-pricing/`)**:

| Surface | URL | One-line behavior (see navigation.md §C for full findings) |
|---|---|---|
| Search | `/settings/corporate-pricing` | 9-col virtualized grid (~591 items); filters Pricebook/Strategy/Location/Currency/Is-Internal/Is-Labor/Active-Only are **server-side on the Search button** (typing only STAGES). Query params: `pricebookName`/`pricingStrategyName`/`currencyId`(USD=1,CAD=2,MXN=3)/`locationNo`/`isInternal`/`isLabor`/`isActive`. |
| Pricing Strategy | dialog on Search/Detail | Add-New is dialog-gated ("New Pricing Strategy"); Save button TEXT flips `Save`→`Saving...`→`Save` — settle on the `"Pricebook saved successfully"` toast, not a disable-wait. |
| Pricebook Detail | `/details/<guid>` | shadcn `<table>` (ID/Product Group Name/Price/New Price/Max Discount), ~2430 rows fully-rendered → content-anchor reads (never count). Save dialog-gated, commits all dirty rows; endpoint `POST /navigator/api/location/pricing/save`. Heavy page → 150s per-test timeout. |
| New Pricebook | `/add?type=equipment\|labor` | React light-DOM; inputs need the native value-setter (`.fill()` no-ops). Save gate = Name + Year + ≥1 strategy. **Create is IRREVERSIBLE (no UI delete) → spec is NO-COMMIT.** |
| Product Group Override | `/settings/corporate-pricing/pg-override` | Equipment\|Labor tabs; **location-gated** (empty until a location is picked). 10 cols incl. **`Active` = Radix checkbox** (LR-036 4th boolean render — read `aria-checked`). Override Price / Max Discount % = click-to-edit cells; Max Discount % capped at 100. Picker is **currency-gated** (NM-1472); Labor data lives on office **1101** (NM-1881), not 1604 (corporate-only — see LR-ENC-005). |
| Toolbar I/O | Search toolbar | ⚠ Export ▾/Import ▾ **drifted 2026-06-10/19** — now open a Year(s)+Currency precondition dialog (was direct CSV / file-chooser). Re-walk before relying; `Loc Pricing Export`(CSV)/`Grid Options`(9-col, persists) unchanged. |

**Module-wide notes**: near-zero `data-testid` (text/role/grid-header/content-anchor selectors); Unicode ✔ booleans on legacy grids + Radix checkbox on Override; **baseline-absent** (no nav2 equivalent — do NOT search the old site for Corporate Pricing). Cold-start gotcha: first authenticated CPR load in a cold Playwright session can render blank (`next-auth CLIENT_FETCH_ERROR`) → re-run. IDs use `TC-CPR-{SRC,STR,DET,NPB,OVR,TIO}-NNN`. Master plan: `plans/pending/PLAN_CORP_PRICING_MASTER.md`.
...
- **MFA**: NONE for the current automation user (no second factor configured — `clients/encore/CLAUDE.md` Automation User Provisioning step 2). `NAVIGATOR_MFA_SECRET` is an optional, currently-unused defensive seed read by `credential-loader.ts`.
- **Session model**: `authenticatedSession` fixture at **`clients/encore/src/fixtures/pages.fixture.ts`** — worker-scoped, fresh login per worker. (Project session setup: `clients/encore/tests/auth.setup.ts`.)
- **OAuth discovery URL**: `https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration` (see **`clients/encore/src/setup/global-setup.ts`**)
- **"Stay signed in?" prompt**: auto-accepted during login (`clients/encore/src/pages/auth/login.page.ts`)
...
- `NAVIGATOR_MFA_SECRET` — optional base32 TOTP seed (unused by the no-2FA automation user)
...
**Grammar (since the 2026-06-11 ID audit)**: `TC-{MOD}-{SUB}-{NNN[A]}` and `BUG-{MOD}-{SUB}-{NNN}`. Codes are registered in **`export_test_cases/module-codes.json`** (single source of truth — mint a new code THERE first; `types.ts` KNOWN_SUB_CODES is parity-asserted against it). Do NOT invent a prefix ad hoc.

| Prefix | Scope | Example |
|---|---|---|
| `TC-LOC-LI-*` | Locations — Local Information tab | `TC-LOC-LI-003` |
...
> Paths below reflect the 2026-06-05 POM restructure (`specs/`→`tests/`, `src/data/testdata/`→`src/data/`, section prefix dropped from src/tests). `<module>` is flat (`locations`, `local-office`, `corporate-pricing`).

- Spec files: `{page-kebab}.spec.ts` at `clients/encore/tests/<module>/`
- Page objects: `{page-kebab}.page.ts` at `clients/encore/src/pages/<module>/` (+ shared `base.page.ts`, `components/`)
- Selectors: `{page-kebab}.ts` at `clients/encore/src/selectors/<module>/` (+ barrel `index.ts`)
- Test data: `{page-kebab}.ts` at `clients/encore/src/data/<module>/` (no `.data` suffix; under `src/`, not `tests/`)
- Shared constants / cross-module: `src/data/common.ts`, shared selectors via the `src/selectors/index.ts` barrel
- Test cases / plans: `specs_planning/test-cases/setup/<module>/` and `specs_planning/test-plans/setup/<module>/`

See `clients/encore/docs/MODULE_REGISTRY.md` for the complete, filesystem-reconciled module→directory mapping.
