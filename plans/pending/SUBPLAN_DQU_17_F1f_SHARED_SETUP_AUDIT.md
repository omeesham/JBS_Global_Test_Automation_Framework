...
> **Largest RT gap, needs discovery**
...
- Existing RT 6% → target >50% per requirements-driven plan.
- Largest outstanding round-trip persistence gap among the 9 modules.
- Location list filtering + selection behavior.
- Save round-trip on any mutable shared-setup state.
- Cross-location cascade (if any selection propagates to siblings).

## Bootstrap

- **Invoke with**: `/execute SUBPLAN_DQU_17_F1f_SHARED_SETUP_AUDIT.md`
...
- **Context files**:
  ...
  - `clients/encore/specs_planning/test-cases/setup/locations/locations_shared_setup_locations_test_cases.md` (do NOT read until Phase 2 diff step)
...
2. Run `npm run validate:fieldinventory-staleness`:
   - **No artifact yet for Shared Setup Locations** → fall through to Phase 1 emit-new-artifact path per LR-013 (b). NOT a HALT.
   - **Artifact ≤14d fresh** → spot-check OK.
   - **Artifact >30d stale** → HALT.
3. Announce: `Browser tool: Playwright CLI. Reason: catalog walkthrough, largest RT discovery, unattended.`
4. **HALT** only on (a) prereq not DONE, or (b) >30d-stale exit.

## Phase 0.5b — Baseline-first walk (LR-045 row 4 / LR-ENC-001 / D11)

Before Phase 1's e2e walk, visit nav2 baseline first and emit `clients/encore/specs_planning/_internal/old-site-baseline/shared-setup-locations-<TODAY>.md`. If an artifact ≤14 days old exists, spot-check 2-3 fields per LR-013; otherwise emit fresh.

1. Chrome Claude (LR-038 v2 auth-heavy row): `navigate` to `https://navigator2.training.psav.com/#/setup/locationdetail/1604`. Locate the equivalent Shared Setup Locations sub-tab.
...
4. Phase 1's field-inventory frontmatter sets `Baseline_Artifact: old-site-baseline/shared-setup-locations-<TODAY>.md`.
...
1. Navigate to `/navigator/locations/1604/settings/location` and click the **Shared Setup Locations** tab.
2. Walk live DOM via Playwright CLI; capture every interactive field, location-list filter, selection cascade.
3. Emit artifact per `field-inventory-spec.md` — **8 mandatory frontmatter keys**:
   ...
   7. `Page_URL: https://cloudapps-e2e.encoreglobal.com/navigator/locations/1604/settings/location` (Shared Setup Locations tab)
...
5. **Priority focus fields**:
   ...
   - Save round-trip on any mutable shared-setup state (the RT-gap-discovery objective).
...
2. Diff Phase 1 artifact against `locations_shared_setup_locations_test_cases.md`.
3. Apply fixes with `**MCP_VERIFICATION_LOG**: shared-setup-locations-<date>.md §<section> "<field>" — <evidence>` citations.
4. File `BUG-SS-NNN.json` per LR-034.
5. Re-export CSV: `npx ts-node export_test_cases/to-csv.ts clients/encore/specs_planning/test-cases/setup/locations/locations_shared_setup_locations_test_cases.md clients/encore/exports/locations_shared_setup_locations_test_cases.csv`. Verify Tags column.
