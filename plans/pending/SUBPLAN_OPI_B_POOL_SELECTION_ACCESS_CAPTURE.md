...
2. Emit `clients/encore/specs_planning/_internal/old-site-baseline/office-pool-capture-<office>-2026-06-04.md` per office (provenance: nav2 intended vs e2e observed, capture date, who/what).
...
4. **Populate `OFFICE_POOL`** in `office-pool.ts` with the confirmed 8. Do NOT add per-office map ENTRIES to tab data files here — C–F LIVE-READ each tab's values per office at migration time (LR-015) and reconcile per the discipline above. OPI_B does NOT pre-capture values (no monolithic script).
...
Adjacent capture-script reuse or inventory nits → DO-NOW (same file) or APPEND grep-verifiable line to the relevant migration subplan (C–F) for the tab whose values were captured. No bare "out of scope".
...
|---|---|---|---|
| HUNTER | `_internal/old-site-baseline/*.md` (per-office capture provenance) | `clients/encore/specs_planning/_internal/old-site-baseline/office-pool-capture-1604-2026-06-04.md`<br>(+ one per confirmed pool office) | `ls clients/encore/specs_planning/_internal/old-site-baseline/office-pool-capture-*-2026-06-04.md` |
| GIVER | test-cases / test-plans / XLSX | (none) — no TC semantics change; only data provenance captured | (none) |
| BUILDER | specs/**/*.spec.ts | (none) — no spec edits in this subplan (migration is C–F) | (none) |
...
| GARDENER | refactor citation | (none) — no monolithic capture script; per-office value capture is per-tab live-read in C–F | (none) |
...
ls clients/encore/specs_planning/_internal/old-site-baseline/office-pool-capture-*.md  # expect: 8 artifacts
...
Confirms and seeds the 8-office pool: structural parity proven per office, the automation user's save-access verified by a setup-time preflight that fails loud on any inaccessible office, and a per-constant inventory of the 78 office-dependent consts (which are values vs flat). `OFFICE_POOL` now holds all 8. Per-office VALUE capture is NOT done here — each migration (C–F) live-reads its tab's values per office (LR-015) and reconciles vs nav2 (F5.1) per the recorded discipline. Next: OPI_C migrates Legal as the pilot and establishes the repeatable per-spec recipe.
