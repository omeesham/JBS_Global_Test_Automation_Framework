...
- [x] [`SUBPLAN_CORP_PRICING_NM2262_LOC_EXPORT.md`](../done/SUBPLAN_CORP_PRICING_NM2262_LOC_EXPORT.md) — **DONE 2026-07-07**: real Loc Pricing Export download round-trip — TC-CPR-TIO-018..024 (018–023 green ×2, 024 data-blocked skip) + TC-012 re-verified; reusable `captureCsvDownload` helper handed off to NM-2264. **Split 2026-07-08 → own submodule `loc_pricing_export` (LEX): `corporate-pricing-loc-export.spec.ts` + `corporate_pricing_loc_export` sheet; TC-018..024 renumbered LEX-001..007.**
...
- [x] [`SUBPLAN_CORP_PRICING_NM2264_EXPORT_ALL.md`](../done/SUBPLAN_CORP_PRICING_NM2264_EXPORT_ALL.md) — Export ▾ 4 variants, drift-fix + real download (Depends: NM2262) — **DONE 2026-07-07**: TC-CPR-TIO-002..005 corrected to the Year(s)+Currency dialog gate; 16 new TCs (025-040) — dialog contract, 1-3 year cap (4th refused), currencyId map USD=1/CAD=2/MXN=3 (live), 4 real per-variant downloads with NM-1997/1998/2005 regressions, Axis-2 combination/result-fidelity/empty-vol DEEP; 22 Export TCs green ×2; reused + extended the NM-2262 download helper (concrete HTTP status). Import ▾ 007-011 drift remains with NM2265. **Split 2026-07-08 → own submodule `export_all` (EXA): `corporate-pricing-export-all.spec.ts` + `corporate_pricing_export_all` sheet; TC-025..040 renumbered EXA-001..016; the baseline TC-002..005 stay in the toolbar_io submodule.**
- [x] [`SUBPLAN_CORP_PRICING_NM2305_LOC_IMPORT.md`](../done/SUBPLAN_CORP_PRICING_NM2305_LOC_IMPORT.md) — loc import real upload + validation (Depends: none; builds upload infra) — **DONE 2026-07-07**: TC-CPR-TIO-041..047 real upload round-trip green ×2 (workers=1) + TC-048 documented large-file boundary; live-verified findings CORRECTED the plan's assumptions — the app auto-submits on file-choose (no Upload click), the import is a per-LOCATION replace (omitted rows deleted, not a per-row merge), and the full-file failure is HTTP 500 "Failed to replace document" / NM-2407 (not the 503/504 timeout tickets); NM-2165/2206 leads classified; upload primitive `uploadFileToOpenDialog` exported for NM2265 reuse (R2 note appended). **Split 2026-07-08 → own submodule `loc_pricing_import` (LIM): `corporate-pricing-loc-import.spec.ts` + `corporate_pricing_loc_import` sheet; TC-041..051 renumbered LIM-001..011 (048→LIM-008, Manual/MD-only).**
- [x] [`SUBPLAN_CORP_PRICING_TICKET_SUBMODULE_SPLIT.md`](../done/SUBPLAN_CORP_PRICING_TICKET_SUBMODULE_SPLIT.md) — **DONE 2026-07-08**: executed the LEX/EXA/LIM split above — 3 real per-ticket submodules (own spec + workbook sheet + delivery branch each), baseline TIO-001..017 retained. Registry + docs + xlsx + ship presets landed; all 8 deliverable branches re-shipped force-with-lease (isolation + shared-consistency proven on remote). Council-reviewed (gpt-5.5) + hand-verified.
- [ ] `SUBPLAN_CORP_PRICING_NM2265_IMPORT_ALL.md` — Import ▾ 4 variants, drift-fix + real upload (Depends: NM2305) — targets its OWN new submodule (`import_all`/IMP) + `corporate-pricing-import-all.spec.ts`; must NOT reuse the vacated TIO-018+ band (split 2026-07-08 into LEX/EXA/LIM). The baseline Import ▾ drift-fix (TC-CPR-TIO-007..011) stays in toolbar_io.
