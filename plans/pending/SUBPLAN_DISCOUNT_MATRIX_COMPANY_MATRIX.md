...
> **Old-site baseline — observation-only, deliberately NOT listed in the machine-gated Concrete-deliverable cell above.** The legacy host refused authentication on all three attempts recorded in the artifact, so the walk enumerated zero controls and the artifact carries `baselineScope: baseline-unreachable`. A zero-control observation cannot be machine-enumerated to the entries schema the denominator gate requires, so listing it as a gated deliverable would assert a machine-complete walk that never happened. The machine-complete walk for this module is the new-site field inventory `clients/encore/specs_planning/_internal/field-inventories/discount-matrix-company-matrix-2026-08-20.md` (gated above). The baseline is retained unchanged as supporting observation and referenced in prose so it stays discoverable but correctly classified — mirroring `plans/done/PLAN_DISCOUNT_OPTIMIZATION_AUTOMATION.md`, which classified its own old-site baseline the same way before closing.
...
15 rows filed into `plans/pending/PLAN_DISCOUNT_MATRIX_DEEP_COVERAGE.md` §1 (D1–D15), each with a named
