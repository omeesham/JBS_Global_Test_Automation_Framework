...
- [ ] **Full suite green at `MAX_WORKERS=2`** — NOTE: this still has all specs on 1604, so a 2-worker run may surface the pre-existing clobber. Acceptance = "no NEW failures introduced by this subplan vs the 2-worker baseline"; document the 2-worker baseline failure set so OPI_C–F can show it shrinking. (This subplan does not claim to fix the clobber — only to add inert machinery.)
