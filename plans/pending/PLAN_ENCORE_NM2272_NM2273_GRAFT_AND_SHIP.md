...
5. **Real E2E proof** (`/graft` Step 3 + LR-059): run `corporate-override/corporate-override-export.spec.ts` and `corporate-override/corporate-override-import.spec.ts` **green ×2** (ticketed to a T0/T1 worker; import TCs use certified office 4107). This proves the **working tree**, which is why the index is synced only after (Phase F), never before. Persist both run summaries.
...
| GIVER | `clients/encore/specs_planning/test-cases/setup/corporate-override/corporate_override_export_test_cases.md` + `corporate_override_import_test_cases.md` + test_plan.md + `clients/encore/testcases/encore_test_cases.xlsx` | merged, de-collided, xlsx regenerated | `npm run check:tc-parity` exit 0 |
| BUILDER | `clients/encore/tests/corporate-override/corporate-override-export.spec.ts` + `corporate-override-import.spec.ts` + page objects/selectors/data + `clients/encore/fixtures/import-all/*.csv` | 22 export + import TCs spliced, green ×2 | `npx playwright test --list corporate-override/corporate-override-export.spec.ts corporate-override/corporate-override-import.spec.ts` resolves all IDs |
| HEALER | NM-2273 partial-success API fix (grafted) | present + covered by an import TC | `npm run check:tc-parity` exit 0 |
| WATCHDOG | NM-2273 remediation subplan artifact is gone; the Phase E audit PASS claim is unverified + `agent-mistakes.md` additions (grafted) | intact, consistent | Phase E audit verdict PASS |
