...
- **D23-1 amendment (2026-08-18, g78-V7)**: D23-1 acceptance was executed as a kind check — magnitudes slipped through. 37 live date inputs riding as 2 keys passed the denominator gate because `keyOf` deduplication collapsed them silently AND the `||` fixpoint break exited the expand loop on `clicked===0` before late-arriving keys settled. Corrected by g78-V7: `||` → `&&`, confirmation scan added, occurrence counts rendered `(×N live)` in the manifest, silent module/office defaults killed.
