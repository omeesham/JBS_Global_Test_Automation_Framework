# SUBPLAN_CORP_PRICING_SHADOW_INTEGRATION — map every field/page dependency in Corporate Pricing (shadow tier — cross-field integration)
...
> **Shadow re-home (2026-06-24, PLAN_CORP_PRICING_JIRA_DELIVERY):** renamed from `SUBPLAN_CORP_PRICING_PRE_EDGE.md` (git history preserved). Re-pointed to the Jira-delivery parent as **shadow-tier** work — the cross-field dependency map is NOT a Jira-ticket deliverable; it runs AFTER the 8 ticket deliverables and is held back from deliverable pushes. Content/depth unchanged. `Depends on` now points at the four surface ticket deliverables (which produce the enriched per-field coverage this map consumes); it `Blocks: SUBPLAN_CORP_PRICING_SHADOW_EDGE.md`.
>
> 🤖 **SESSION BOOTSTRAP — invoke with `/execute SUBPLAN_CORP_PRICING_SHADOW_INTEGRATION.md`. All context below; zero extra prompting.**
>
...
**Depends on**: SUBPLAN_CORP_PRICING_NM2260_FILTERS_DETAIL.md, SUBPLAN_CORP_PRICING_NM2261_STRATEGY.md, SUBPLAN_CORP_PRICING_NM2263_NEW_PRICEBOOKS.md, SUBPLAN_CORP_PRICING_NM2267_OVERRIDE.md
**Blocks**: SUBPLAN_CORP_PRICING_SHADOW_EDGE.md
...
> **Cross-ref (2026-06-24, SUBPLAN_CGS_A):** the cross-field/integration *technique* this map feeds is
> now codified generically in the [Case-Generation Standard](../../docs/read_only_docs/CASE_GENERATION_STANDARD.md)
> L3 DEEP model (integration / cross-field family) — `/ultracoverage` consumes a dependency-map artifact
> for exactly this. **This subplan is NOT superseded or deleted** — its product (the Corporate Pricing
> dependency-map artifact) is module-specific live-walk output that the generic Standard cannot produce;
> it stays PENDING as a distinct deliverable AND as the `Blocks: SUBPLAN_CORP_PRICING_EDGE_P3.md`
> dependency. On activation, EDGE_P3's DEEP authoring should run via `/ultracoverage corporate-pricing`
...
1. Write `clients/encore/specs_planning/_internal/corporate-pricing-dependency-map-<session-date>.md` (dated per the `_internal` walk-artifact convention; sibling to walk-evidence). Structure:
   ...
   - **Cross-page data-flow section**: directed edges (page A field → page B field) with trigger + evidence — **each edge tagged with its CGS surface family** (`combination` for cross-surface filter/sort/compute interactions, `persistence` for cross-page dirty-state / save-survival) so `SUBPLAN_CORP_PRICING_SHADOW_EDGE` authors the inter-surface `TC-CPR-SBC-MAX-*` cases directly from this map. The per-ticket SBC tables in the 8 deliverables cover within-surface families only; **this map is the SOLE enumeration of the cross-surface layer** (the families that structurally cannot be seen from inside one ticket's grid).
