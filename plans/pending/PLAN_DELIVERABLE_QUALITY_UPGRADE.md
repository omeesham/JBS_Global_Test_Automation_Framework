...
| D12 | **Mandatory depth coverage grid in every audit findings doc** — rubric source-of-truth is `clients/encore/specs_planning/_internal/neutral-eye-audits/_TEMPLATE.md` §Coverage depth grid + §Coverage matrix (fully-qualified path; do NOT confuse with `clients/encore/specs_planning/_internal/field-inventories/_TEMPLATE.md` which has no coverage rubric). Applies regardless of artifact format the audit emits — neutral-eye findings doc OR field-inventory artifact, the coverage matrix is mandatory output. Every interactive field cross-checked against ISTQB-standard required-TC list (BVA + equivalence partitioning + cross-field + error-guessing) before suggesting new TCs. GAP count > 50 → escalate per LR-040. SP-DQU-05E (NEW) retro-applies this to LOS + LI before HIST starts. | User directive 2026-04-29: "we need enough test cases that cover our ass for these 2 modules such that when we do hist later, we have full in depth coverage". Inline depth rules in template > research subplan (SP-DQU-10 stays P2; depth doesn't block). Path disambiguation added 2026-04-29 post auto-injection health check (Gap A). Locked 2026-04-29. |
...
## Inherited registry-hygiene item (SUBPLAN_GUARDRAIL_CX_GATE_DEFECTS Phase 2.5 sweep, 2026-08-28)

- **case-rows.json staleness class** — the per-artifact case-row registry (`reports/walk-coverage/case-rows.json`, local-only by the walk-evidence policy) rots silently when the taxonomy grows: stale emissions convicted the parity check on two module closures (NM-3344 retroactively, NM-3530 at closure) before a data refresh cleared both. Three line items: (1) refresh the stale `dop-tab1` emission under the current taxonomy — its inventory pairing was ambiguous on 2026-08-28, locate the pairing first; (2) purge orphaned `source_artifact` row-sets whose completion record no longer exists on disk (`1604-discount-matrix`, superseded by the three dsm-* records); (3) consider a staleness guard coupling each emission to a taxonomy fingerprint so a stale registry names ITSELF instead of failing parity with a misleading A≠B.

## Inherited layout-gate items (PLAN_NM2253 closure sweep, 2026-09-01)

- **accumulator-SHA-pin false-tamper class** — `SUBPLAN_GUARDRAIL_CX_GATE_DEFECTS`'s closure manifest pinned `reports/walk-coverage/case-rows.json`'s sha256 in its `artifacts[]`; the NM-2253 closure's two legitimate `emit-case-rows --merge` emissions (isr-pcd view/add, all prior rows kept) changed the file, so `validate-plan-layout.mjs --check` now reports `[RED] Artifact tampered` against that DONE plan — and every future module's emission re-trips it. A living accumulator must not be SHA-pinned in a closure manifest (or the layout tamper check needs an accumulator-artifact carve-out that validates row-set preservation instead of byte identity). Do NOT regenerate the old plan's manifest to silence it — that rewrites closure evidence; fix the gate design.
- **layout Status parse window vs bootstrap-first template** — `validate-plan-layout.mjs` reads `Status` from the first 2000 chars, but the NM-3530/NM-2253 plan mold puts the SESSION BOOTSTRAP block before frontmatter (`**Status**: DONE` lands at offset ~2700), so both plans read as `non-terminal Status: missing` (advisory YELLOW, same class across the mold). Either widen the header window / parse to the first frontmatter block, or move Status above the bootstrap in the template.
