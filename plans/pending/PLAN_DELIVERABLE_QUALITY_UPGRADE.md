...
| D9 | /simplify and /cleanup scope: Encore deliverables + Encore-specific runtime code ONLY. Whitelist: specs under `tests/specs/`, page objects under `src/pages/`, selectors under `src/selectors/`, test data under `tests/test-data/`, test-case MDs under `clients/encore/specs_planning/test-cases/`, exports under `clients/encore/exports/`, export script `export_test_cases/to-csv.ts`. Out of scope: `.github/agents/`, `.claude/`, website/, tools/, config/, orchestrator, pipeline. | User explicit. |
...
11. **Deliverable package**: `clients/encore/exports/` + Allure report + bug reports bundled for client.
