...
2. Author and file `SUBPLAN_DQU_19A_ECT_BOOTSTRAP_TC_MD.md` first to:
   ...
   - Run `npm run xlsx:build` so the new `ect_settings` TCs land as a sheet in `clients/encore/test_cases_xlsx/encore_test_cases.xlsx` (the per-module CSV initial-export was retired in PLAN_CSV_TO_XLSX_DELIVERABLE_MIGRATION Phase D).
