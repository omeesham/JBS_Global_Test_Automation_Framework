...
- [x] Location Products (distinct surface): **QUICK tier DONE 2026-09-01** — `PLAN_NM2253_ITEM_SEARCH_COVERAGE_QUICK.md` closed (41 TCs ISR PRS21/PCD10/PGR10, module run-all 42 passed; BUG-ISR-PCD-001 + BUG-ISR-PRS-001 filed; BUG-ISR-PCD-001 later INVALIDATED same day by owner-triggered LR-044 re-verification — Category opens fine, walk reading was a stale-ref instrument zero; TC-ISR-PCD-005 extended + green; 42 elements seeded into `SUBPLAN_PRODUCTS_DQU.md`; fresh-session audit pending in `PLAN_NM2253_ITEM_SEARCH_EXTERNAL_01_AUDIT.md`).
  - [SUBPLAN_ITEM_SEARCH_BARCODE_COVERAGE.md](../done/SUBPLAN_ITEM_SEARCH_BARCODE_COVERAGE.md) — **DONE 2026-09-01**, barcode search covered end-to-end on twelve owner-supplied numbers (TC-ISR-PRS-022…030, module run-all 51 passed); OR-4 closed; the Any Field ↔ Barcode exclusivity gap found and covered; barcode-vs-other-criteria deferred to `SUBPLAN_PRODUCTS_DQU.md`. Remaining checkbox scope = the DQU DEEP tier (`SUBPLAN_PRODUCTS_00_FOUNDATION` + `_FCC` stayed SUPERSEDED, never executed).
  - [SUBPLAN_ITEM_SEARCH_SAVE_FLOWS.md](../done/SUBPLAN_ITEM_SEARCH_SAVE_FLOWS.md) — **DONE 2026-09-02**, save half of NM-2253 closed: NM-2254 Active-filter effect (TC-ISR-PRS-031), NM-2257 Add Product Code real save (TC-ISR-PCD-011), NM-2259 Create Product Group real save (TC-ISR-APG-004), each proven by reload-and-read (LR-067); NM-2256 View Availability re-driven to app-gated-inert behind the dates ruling (no TC); the four non-Item Add segments waived as out-of-epic catalog-hierarchy creates (LR-066); the LR-066 parity registry rewired to register Item Search (registry-opt-in surfaced as an owner finding); item-search run-all 54 passed, 0 failed.
