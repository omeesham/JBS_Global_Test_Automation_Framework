...
The genuine residue is small, already found, and already owned by `PLAN_74` Links 6–7: the walk artifact's header asserts `Coverage_Ratio: 29/29 (100%)` / `CrossCheck: clean` over machine data that disagrees, and the real module-surface subset inside the miscount still needs dispositioning. PLAN_74 names both as "two separable jobs, and they must not be conflated". They stay there. This plan does not re-open, re-file, or duplicate them.
