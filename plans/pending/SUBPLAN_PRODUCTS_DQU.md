...
- [ ] PRS barcode versus the other criteria: the QUICK wave (TC-ISR-PRS-022…030) proved the barcode box against itself and against Any Field, but never against the Location / Region / date / checkbox criteria — does a barcode search ignore them, or narrow with them? Add a case that searches a known barcode with a conflicting Location and one with the Quantity filter on, and record which side wins (deferred-to-DEEP at the 2026-09-01 barcode wave; the supplied barcodes belong to office 1121 assets while the page runs on 1101, so an office-versus-barcode conflict is directly testable)
