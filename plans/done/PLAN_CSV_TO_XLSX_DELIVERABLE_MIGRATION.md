...
- **MODIFY** `export_test_cases/to-xlsx.ts` — switch from local parser fork to shared `markdown-parser.ts` now that types are aligned. **DEFERRED to spawned task** (2026-05-27, user-authorized per LR-046): the parser switch + `convertStepsToHuman` integration is a 2-3hr surgical refactor that requires mapping the shared `MarkdownParser.parseMarkdownFile()` → `ParsedTc` shape and iteratively closing ~1127 cell diffs against the CSV oracle. Filed as a `spawn_task` chip ("XLSX list-only parser switch (Phase D pre-flight)") so the main migration session can land A.5/B/C without scope explosion. Phase D pre-flight asserts the spawned task has landed by checking `npm run xlsx:build:list-only && npm run xlsx:vs-csv-parity` exits 0 BEFORE any CSV deletion (HALT otherwise). The `--from-csv` default in `package.json` REMAINS through A.5/B/C; the spawned task flips it to `--list-only` when it lands.
- **PORTED** `export_test_cases/humanize.ts` — shared helpers extracted from `to-csv.ts:383-597` (cleanMarkdown, humanizeAssertion, convertElementIdsToLabels, generatePreconditions, convertPreconditionsToHuman, convertStepsToHuman, convertExpectedToHuman, TAB_MAP, camelToLabel, sanitizeUnicode, humanize composite). Landed in Phase A.5 so the spawned parser-switch task can consume them without further extraction work. `to-xlsx.ts` imports `humanize` and applies it inside its current `parseMd()` local fork (partial coverage; full parity awaits the parser switch).
...
- **VERIFY**:
  1. `npx tsc --noEmit -p tsconfig.json` shows 0 NEW errors in `export_test_cases/*.ts` after the N1 rename (pre-existing errors in unrelated scripts are out of scope)
  2. `JsonConverter.convert(...)` programmatic call produces valid output; metadata key is `pendingAutomation` (no `npm run export:json` script exists in `package.json` — the converters are invoked programmatically via `export_test_cases/index.ts` barrel; smoke-checked via `ts-node -e`)
  3. `JiraConverter.convert(...)` produces valid Jira CSV; `Test Type` column AND `Automation Status` column both emit `Manual` for non-Automated TCs (external compat via `toJiraTestType` + `toJiraAutomationStatus` boundary mappers added in this phase)
  4. `TestmoConverter.convert(...)` produces valid TestMo JSON (no source change needed — `mapStatus` already translates the union to ready/in_progress/draft; `automation_status` custom field now emits the new internal token, which is acceptable for TestMo's free-form custom fields)
  5. `npm run xlsx:build` (still default `--from-csv` in A.5; spawned task flips to `--list-only`) → exit 0
  6. `npm run xlsx:vs-csv-parity` (against `--from-csv` build) → exit 0
  7. Grep for naked literal `'Manual'` in framework code (excluding allowlist + sibling-exporter mapping + Jira/TestMo external vocab + parser backwards-compat shim + this plan body) returns 0 hits
