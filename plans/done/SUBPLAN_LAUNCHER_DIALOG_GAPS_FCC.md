...
4. Prevention is mandatory and permanent: "from now on this fuckup should not be caused" — including the no-taxonomy-row HALT backstop (all cases per field, never "call the walk as end" with unfilled cells).
...
## Phase 5.5 — Prevention encoding (OWNER; user mandate "from now on this fuckup should not be caused")
