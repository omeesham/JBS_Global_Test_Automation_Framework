...
| the module-wide field inventory dated 2026-08-25 *(superseded — split per D-22 into the three per-submodule artifacts named below; the original file no longer exists)* | **EXISTS** *(at authoring)* — 35 union rows, `Walk_Mode: quick`, `CrossCheck: clean`, `Coverage_Ratio: 12/35 (34%)`. Phase 6b is **done**; the 23 open rows are the ones Phase 1 of this plan claims. *(State as at plan authoring. All 35 rows were dispositioned during execution, and on 2026-08-27 the artifact was split into one inventory per submodule — `discount-matrix-criteria-2026-08-25.md` 17/17, `discount-matrix-region-weekly-peaks-2026-08-25.md` 20/20, `discount-matrix-location-activation-2026-08-25.md` 20/20 — to match how every other module pairs one inventory per test-case file. See D-22.)* |
...
- [x] Phase 0.5b baseline walk EXECUTED before any behaviour classification or bug filing (Gate 1 — LR-045 / LR-ENC-001). *(Phase 0.3 ran 2026-08-25; `baselineScope: complete`; classifications carry real `baselineComparison`.)*
- [x] No "corrupt / atypical / app-wide / regression" claim on <2 evidence sources — 2 offices OR new-site+baseline (Gate 2 — LR-061-A). *(N≥2 across 1604+1101 throughout; negative claims additionally varied-wait retried per LR-ENC-008.)*
- [x] No control marked un-drivable without overlay-clear + reload + selector-vs-live-DOM diff + DOM inspect (Gate 3). *(No un-drivable verdict stands — the two "inert" candidates were re-derived and overturned.)*
- [x] No environment-rationalised deferral of environment-independent work (Gate 4 — LR-060). *(All deferrals are mutation-risk or tier-scope, none cite env.)*
- [x] Un-skip + LR-019 harden applied atomically in the same change (Gate 5). *(TC-DSM-LOA-010/011 un-fixme'd and re-authored to the owner-ruled contracts atomically with the bug withdrawals, 2026-08-26.)*
- [x] All phases complete OR a user-signed `## Deferral Authorization` block recorded (Gate 6 — LR-060). *(All phases complete; the Deferral Authorization section is empty because nothing needed signing — deep-tier deferrals are LR-072 dispositions inherited by the parent plan, not phase skips.)*
...
3. Refresh `clients/encore/specs_planning/_internal/old-site-baseline/discount-matrix-2026-08-25.md`: flip `baselineScope: partial` → `full` (or record precisely what remains uncaptured and why), and extend the `## Baseline diff` section with RWP and LOA rows.
...
| WATCHDOG | completeness + bug-loop-closure findings | `clients/encore/specs_planning/_internal/audit-discount-matrix-2026-08-26.md` | `npm run check:spec-quality` |
...
- [x] Jira crossref extended with NM-3074, NM-2220, NM-2221, NM-1657, NM-1681, each marked as a lead. *(Completed at closure 2026-08-28 — the closure audit found the append had never landed although the sweep's content was used and DOM-verified during execution; appended with live re-verified statuses (all five now Done) and the artifact's stale open-question section resolved on the record. See D-24.)*
- [x] Old-site baseline artifact carries a `## Baseline diff` covering RWP and LOA, and `baselineScope` reflects what was actually captured. *(Verified 2026-08-28: `baselineScope: complete (CRT + RWP + LOA all captured)`; the diff section covers both tabs.)*
- [x] The LOA verdict contradiction is resolved on the record — the superseded reading named, the new evidence cited, N≥2 across offices 1604 and 1101. *(Dated correction paragraphs in the walk evidence; the crossref's escalation withdrawal recorded 2026-08-28; the class graduated as LR-ENC-008.)*
- [x] The **union denominator of 35** is fully dispositioned (from `12/35` at authoring time), `CrossCheck: clean`, `Coverage_Ratio` 100%, opener frontier empty (LR-062 / §20 / Cx). One field inventory, updated — not a second dated artifact alongside it. *(Satisfied as amended by owner-approved D-22: the single inventory was split into three per-submodule artifacts matching the repo convention — 17/17 + 20/20 + 20/20 reconciling exactly to the 35-row union with shared chrome counted once; no second dated artifact of the same scope exists.)*
...
- [x] `Walk_Mode: quick` stamped on the walk artifacts and matching this plan's `CoverageMode: quick` — the Cx gate fails on a mismatch. *(Verified 2026-08-28 on all three inventories.)*
- [x] Every `deferred-to-DEEP` row names a specific element/launcher with a reason ≥20 chars and carries no other disposition token. *(8 deferrals, LR-072 G1-clean; enumerated grep-verifiably in the parent plan's scope lock 2026-08-28 for LR-040(b).)*
- [x] Walk-evidence carries `## Observations` with both buckets filled or the literal `none` (ALL-045). *(Both buckets present, incl. the 2026-08-27 criteria-bar probe section's additions.)*
- [x] Every walk-found bug filed per LR-034 with `baselineComparison` + numbered `stepsToReproduce`, and has a corresponding TC; every skip names its bug ID. *(BUG-DSM-LOA-001/-002 filed per LR-034, both withdrawn on owner rulings 2026-08-26; TC-DSM-LOA-010/011 assert the ruled contracts; zero skips remain in the suite.)*
- [x] The NM-3238 intended model (mutual exclusivity vs independent toggling) is settled from live evidence or explicitly routed to `/encore-questions` — never assumed — **and the RWP peak-flag §2 template was assigned after that resolution, not before**. *(Settled live: mutually exclusive, radio-like; cases authored after resolution.)*
- [x] `Add Year` and `Import` reversibility established BEFORE any positive case ran; if irreversible, the creation case sits in `## Deferral Authorization` with a signature, not in the suite. No speculative mutation of office 1604 to find out. *(Probed first: creation is permanent server-side; the canonical create TC-DSM-RWP-022 ran 2026-08-26 under the owner's explicit mutation authorization — a stronger instrument than a deferral signature; deviating create paths deferred-to-DEEP as `rwp-create-year-variants`.)*
- [x] Load-cost strategy recorded, per-test timeout set to accommodate the measured ~134s load, and the finished suite's wall-clock stated in the Execution Summary. No `waitForTimeout` substituted for a raised timeout, and no readiness bar lowered to make a test fast. *(Serial-per-surface with explicit value-reset restoration; wall-clock in the Execution Summary: ~1.0h solo cold, 12.6m warm inside the full regression.)*
- [x] Every row carries an `affordance:` token; no inert verdict without a positive control. *(Audited — `audit-discount-matrix-2026-08-26.md`.)*
- [x] Every zero-delta probe on a mandatory-effect class carries `DIFFERENTIAL-DATA-REQUIRED` with rung-1 and rung-2 evidence; no case asserts a zero-effect as expected behaviour. *(The one zero-delta candidate — LOA search "filters nothing" — was re-derived as a loading-window artifact per LR-ENC-008 and its bug withdrawn; no case asserts a zero-effect.)*
- [x] Every applicable §3 family carries a QUICK TC or `out-of-scope:<family>=<reason ≥20 chars>`. *(Catalog §3 tables complete for all three surfaces, incl. the 2026-08-27 per-tab-state re-query row.)*
- [x] Date BVA covered on LOA's Workflow Start Date, or explicitly `out-of-scope:date-bva=<reason>` — never faked. *(Dispositioned honestly via the quick-tier token instead: `deferred-to-DEEP: loa-workflow-start-date-cell-editor` — persisting shared-data grid mutation; QUICK records the affordance contract only; inherited by the parent plan.)*
- [x] Missing-testid findings recorded with live-DOM verification per element (LR-029); no DOM/markup accessibility finding filed as a bug, TC or observation. *(The scope ruling is now numbered LR-ENC-009, graduated at closure.)*
- [x] No `(QUICK)`/`(DEEP)` marker on any `## TC-…:` heading (ALL-091). *(xlsx-lint enforces at build and commit; green at d0495d1.)*
- [x] No `networkidle`, no `page.waitForTimeout`, no row-count-based ready-gate anywhere in the new specs. *(Spec-quality battery green on the working tree and at every commit gate; readiness is skeleton-census / control-count based.)*
- [x] `npm run check:tc-parity`, `lint:testcases`, `xlsx:lint`, `typecheck` all exit 0. *(All green during execution sessions; tc-parity + the xlsx gates re-ran green in d0495d1's pre-commit battery 2026-08-28, and no input of theirs changed after that commit — closure edits touch plans/registry/config only.)*
- [x] `npm run check:spec-quality` passes on the working tree before any done / green / verified claim. *(Ran green before the green claim on 2026-08-27 and again inside the d0495d1 commit battery.)*
- [x] Suite green ×2 consecutively; every mutating case restores state; re-runs idempotent. *(Run-all 65/65 green 2026-08-26 and 69/69 green 0-retry/0-skip inside the 1,143-test full regression 2026-08-28, plus solo 4/4 for the new cases; per-test-baseline gate green; re-runs idempotent across sessions.)*
- [x] `/regression-guard` before/after shows no silent breakage. *(Guard snapshots during execution sessions; final full-regression evidence: every non-DSM failure in the 2026-08-28 run catalogued pre-existing/out-of-scope in D-23 — none caused by this plan's files.)*
- [x] `PLAN_DISCOUNT_MATRIX_AUTOMATION.md` gated and its scope lock amended — no two pending plans claim the same work. *(GATED + carve-out + deep-tier deferral enumeration added 2026-08-28.)*
- [x] LR-028 activity-log row with an LR-037 timestamp ≥ every touched-file mtime. *(Rows 2135–2152 across execution sessions; closure row appended 2026-08-28 with preflight green.)*
- [x] `/final-q` verdict block emitted per LR-042. *(Emitted in chat at closure, 2026-08-28.)*
...
| D-23 | The owner asked (2026-08-27, from the three tab screenshots) whether the criteria bar was tested on all three submodules. Honest answer: no — the CRT band drives the bar on the landing tab only; on RWP/LOA the specs merely read resting values, yet the split inventories claimed 100% by discharging the shared chrome "in the sibling criteria artifact". Coverage-by-reference, not execution (CEO-M13, Sev S1, + amendment: the LOA plan HAD consciously deferred the country re-query to the deep tier, so the artifacts contradicted each other and no gate compared them; the other three sub-gaps were genuine misses) | Owner ordered the gap closed ("add cases for the criteria bar on RWP and LOA tabs"). Auth state had expired — refreshed via the unattended setup. Two scripted probes measured the axis live: `reports/walk-coverage/dsm-critbar-rwp-probe.json` + `dsm-critbar-loa-probe.json` — the tab holds focus on a country change; Currency cascades with Country both ways; RWP re-scopes (Canadian region list, empty year selector on this server) and restores byte-equal on switch-back; the LOA listing swaps US↔Canadian sets and restores anchor-first; a threshold edit enables ONLY the bar Save (panel Save independent); a dirty bar rides tab switches with no dialog; a bar save clicked from RWP commits (POST 200 + reload-proven); threshold restored to 15% verified. Four cases authored across all four layers: TC-DSM-RWP-025/-026 + TC-DSM-LOA-012/-013 — MD + test plans + catalog + all three inventories re-worded to name the state each covering TC drives; the LOA plan's deep-tier deferral row retired; two honest deferrals recorded (currency/tier-from-tab; the landing tab's own grid re-scope effect, which no CRT case asserts either — CRT-007's oracle is the dropdown value). Module 65 → 69 cases. Verification: tsc clean; workbook rebuilt; tc-parity PASS; spec-quality PASS; per-test-baseline PASS (the ensureClean beforeEach in both describes already satisfies the mechanism); solo 4/4 green 0-retry (5.0m); then — an EXECUTION ERROR turned into extra evidence: the intended DSM-suite run was launched unscoped, running the ENTIRE client regression (1143 tests, 3h20m, junit `tests=1143 failures=19 errors=13 skipped=39`). Within it, **discount-matrix: 69/69 passed, 0 failed, 0 skipped, 0 errors** — the strongest run-all proof yet, with cross-MODULE inherited state ahead of the DSM specs. The 19f/13e are all in other modules (locations, corporate-override, local-office, service-charge-text, terms-conditions), out of NM-3530 scope, untouched by this plan's edits (which are DSM-only), recorded here as a health snapshot for their own plans. Two run-observability defects were mine and are logged for the record: the run was piped through `tail -6` (live output+failure monitor blind until exit) and the pre-run clean targeted wrong paths (root-level names; real dirs are `reports/allure-results` etc.), so allure accumulation persists — irrelevant to the delivered Playwright HTML report at `clients/encore/reports/html-report/` (regenerated per run, written 00:34). The json reporter crashed in its end hook this run (stack in the task log; `clients/encore/reports/test-results.json` is STALE at the solo-run values) — junit + HTML + allure are the authoritative artifacts for this run |
| D-24 | The closure audit (2026-08-28) found Phase 0.1 had never landed on disk: the Jira crossref still carried only the original 18 tickets — the five leads (NM-3074, NM-2220, NM-2221, NM-1657, NM-1681) whose intent content this plan embeds and whose contracts the passing cases DOM-verified were absent from the artifact — and the crossref's "Open question" section still asserted the superseded zero-locations reading whose `/encore-questions` escalation Phase 0.2 had ordered withdrawn | Completed at closure as OWNER (the artifact's own frontmatter grants Owner RW): the five leads appended with statuses re-verified live via Atlassian MCP on 2026-08-28 (all five now Done — NM-3074 was QA at the 2026-08-25 sweep and has since closed), each row carrying its DOM-verification pointer; the open question resolved with a dated RESOLVED block naming the loading-window misread (LR-ENC-008) and the 2026-08-26 withdrawal. The lesson already captured as the correction-pass-sweeps-every-artifact memory is re-confirmed: the closure-time grep is what caught both |
| D-25 | The DONE flip is HELD (2026-08-28). The closure validator's Cx (coverage_mode=deny) fails on three gate-side reasons after every artifact-side fix landed: (1) the denominator-parity engine fires with a dead exemption channel — `reconcile()` at `scripts/walk-coverage/verify-denominator.mjs:445` passes no exemptions, Path B is empty-by-construction for QUICK-tier artifacts, and the same check retroactively fails the DONE service-charge precedent (`plans/done/PLAN_NM3344_SERVICE_CHARGE_COVERAGE_QUICK.md`, flipped 2026-08-15); (2) the old-site baseline is demanded a machine coverage manifest that the owner-authorized bounded observation walk could never produce; (3) the owner-signed 15%-cap exemption (Phase 0.05 route (b), 2026-08-26) has no machine hook — criteria reads 7/17 in-module OOS with no lawful escape for the sibling-owned CMX rows | Owner ruling in chat (2026-08-28): waive the three checks by name with dated notes, leave `coverage_mode=deny` alone, file the guardrail plan, and STOP if per-check waiving is unsupported. It is unsupported (verified in code — no per-check waiver surface exists in `closure-overrides.json` [C1-only], `walk-exemptions.json` [halted/unreachable shapes only], or the Cx path), so the flip stopped per the ruling. Fixes that DID land this session, all verified by re-running the dry-run: four C3 citation repairs, the Cr glob reword, ten elided manifest keys re-bound verbatim across the RWP/LOA inventories, and the two split-module registrations in `scripts/walk-coverage/lib/module-config.mjs` — the key-drift, unregistered-module, unresolved-probe and C3 classes are all clear. The three residual defects are filed with fix directions and negative-test acceptance in `plans/done/SUBPLAN_GUARDRAIL_CX_GATE_DEFECTS.md` (filed to pending/ on 2026-08-28, executed and moved to done/ the same day — grep-verifiable recipient per LR-040(b)); that plan's completion re-runs this closure — only the Status flip + `git mv` + reindex remain (completed, see D-26) |
| D-26 | Completion of the D-25 hold — the guardrail plan (`plans/done/SUBPLAN_GUARDRAIL_CX_GATE_DEFECTS.md`) executed and closed 2026-08-28, unblocking this flip | Defect 1 resolved by DATA, not engine surgery: Phase-1 research exonerated the parity check (the case-row emitter deliberately shares Path A's widened-union definition) — the real fault was a stale local `reports/walk-coverage/case-rows.json` (old taxonomy; the three re-enumerated DSM records had never emitted); five `--merge` re-emissions restored EXACT A==B parity everywhere and un-failed the NM-3344 precedent too (guardrail D-1 + CEO-M14). Defect 2: observation-only old-site carve-out (old-site-baseline path + `Observation_Only` + `Walk_Authorization` ≥20 chars) in `scripts/walk-coverage/lib/coverage-manifest.mjs`; the DSM baseline now passes citing the real 2026-08-25 grant. Defect 3: machine hook consuming the owner-signed 2026-08-26 route-(b) cap exemption from `.claude/walk-exemptions.json` `oos_cap_exemptions`; unsigned entries refused. Waiving the cap un-masked the criteria inventory's own 4 elided manifest keys + missing module registration (the cap failure had short-circuited the denominator checks) — fixed in the same pass. Gate self-test 42/42 (7 new fixtures, every carve-out with a refusing twin); this plan re-validated **[PASS]** under `coverage_mode=deny` (untouched per the owner ruling); flip completed 2026-08-28 |
...
**Executed**: 2026-08-25 → 2026-08-28 (intake inherited from the parent plan; walk reconciliation, authoring, specs, audit, criteria-bar gap closure and plan closure across the sessions of 2026-08-25/26/27/28).

**TCs implemented — 69 of 69 authored, zero dropped, zero skipped**:

- CRT `TC-DSM-CRT-001..030` (30) — criteria-bar field + surface cases: the three unlabeled comboboxes, the formatted GAV threshold contract (char-by-char typing, four silent coercions, read-then-restore, NM-3441/3440/3294/3391 leads), persistence, dirty-guard, and the bar's own Save.
- RWP `TC-DSM-RWP-001..026` (26) — Year/Region cascade, peak-flag mutual exclusivity (NM-3238 model settled live before template assignment), `Add Year` end-to-end (RWP-022, run 2026-08-26 under the owner's explicit mutation authorization), export download (RWP-023), import dialog, the NM-3485-corrected Cancel contract, empty-vol on the NM-3293 region, and the 2026-08-27 cross-tab additions RWP-025/026 (criteria re-scope + restore; threshold save from this tab).
- LOA `TC-DSM-LOA-001..013` (13) — country-scoped listing per NM-2221, footer count oracle, `Active` toggle archetype, dirty/discard (LOA-009), steady-state search result-fidelity (LOA-010), display-only headers (LOA-011), and the cross-tab additions LOA-012/013 (listing swap anchor-first; threshold save from this tab).
- The two once-fixme'd cases were re-authored to the owner-ruled contracts atomically with their bug withdrawals (2026-08-26); the shipped suite carries no fixme and no skip.

**TCs dropped**: none. The planned-but-unauthored remainder is 8 deep-tier deferrals dispositioned `deferred-to-DEEP` per LR-072 (G1-clean, reasons ≥20 chars) and enumerated grep-verifiably in the GATED parent plan's scope lock on 2026-08-28 — `rwp-create-year-variants`, `loa-search-input-contract`, `loa-workflow-start-date-cell-editor`, `loa-persistence-save-path`, `critbar-currency-tier-per-tab` (inventory split `-on-rwp`/`-on-loa`), `cmx-grid-requery-effect`, the export cell-level read-back, and the More-information popover open — satisfying LR-040(b).

**Verification results (numbered)**:

1. Machine denominator 35/35 dispositioned across the three per-submodule inventories (17/17 + 20/20 + 20/20, shared chrome counted once — owner-approved split, D-22); `CrossCheck: clean`, `Coverage_Ratio` 100%, opener frontier empty; the 15% out-of-scope cap resolved by owner-signed route (b), 2026-08-26.
2. Phase 0.2 re-derived the contradicted LOA verdict from post-fix machine evidence: populated (2041 US locations, ~43s tab-load), N≥2 offices; `data-blocked` overturned with dated corrections; the `/encore-questions` escalation withdrawn (recorded in the crossref at closure per D-24). The doctrine item — verdicts written from pre-fix evidence are re-derived, not inherited — graduated to auto-memory at closure.
3. Old-site baseline: `baselineScope: complete (CRT + RWP + LOA all captured)`; `## Baseline diff` covers both tabs; divergences classified.
4. Criteria-bar cross-tab contracts measured live 2026-08-27 (probe records `dsm-critbar-rwp-probe.json` / `dsm-critbar-loa-probe.json`, cited by the walk evidence): the tab holds focus on a Country change; Currency cascades both ways; RWP re-scopes and restores byte-equal; the LOA listing swaps US↔Canadian anchor-first; a threshold edit enables only the bar's Save, rides tab switches with no dialog, saves from a non-landing tab (POST 200 + reload-proven), and the 15% baseline restores clean.
5. Suite green ×2: run-all 65/65 (2026-08-26, 0 retries) and 69/69 with 0 retries / 0 skips inside the 1,143-test full client regression (2026-08-28, junit-verified per-suite split), plus solo 4/4 (5.0m) for the new cases. Module wall-clock ~1.0h solo at cold hydration; 12.6m warm inside the full run — recorded per the load-cost criterion; no readiness bar was lowered.
6. Gates: tc-parity, xlsx freshness + fingerprint, field-inventory pairing, dead-exports, spec-quality, structural-names and the LR-037 activity-log preflight all green at commits `8f10554` / `86b47bf` / `d0495d1`; typecheck green in-session; per-test-baseline gate green.
7. Closure-time audit findings, both fixed 2026-08-28 (D-24): the Phase 0.1 crossref append had never landed, and the crossref's open-question section still carried the superseded reading — completed and resolved on the record.

**Bugs**: BUG-DSM-LOA-001 (search dead-window) and BUG-DSM-LOA-002 (headers not sortable) — filed per LR-034 with `baselineComparison` + numbered steps, both withdrawn on owner rulings 2026-08-26 (accepted loading behaviour / display-only by design); the ruled contracts are asserted by TC-DSM-LOA-010/011. No open module bugs. NM-3234's fix is covered as expected behaviour; nothing was re-filed.

**Documentation changes**: MODULE_REGISTRY promotion (2026-08-26); REQUIREMENTS.md covered-behaviour rows; Exploration Registry row brought to closure state (2026-08-28, stale bug references corrected); parent plan GATED with the amended scope lock + deferral inheritance; LR-ENC-008 graduated 2026-08-26; LR-ENC-009 numbered at closure (the 2026-08-04 accessibility-scope ruling, resolving the documented ALL-045 conflict); the readiness-fix re-derivation doctrine graduated to auto-memory; the Jira crossref completed per D-24.

**Test pass confirmation**: 69/69 green, 0 retries, 0 skips, 2026-08-28 — `clients/encore/reports/junit-results.xml` (1,143-test run; DSM split t=69 f=0 sk=0 e=0 by scripted junit parse) + the HTML report at `clients/encore/reports/html-report/`.
