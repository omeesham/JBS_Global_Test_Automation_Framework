...
**Stage C — ADVERSARIAL VERIFY (per verdict; ≥3 independent, perspective-diverse skeptics).** Every Stage-B **PASS** → skeptics prompted to REFUTE "this corner is clean" (default *refuted* if uncertain); survives only if a majority cannot break it. Every **FAIL** confirmed real. The 1%-fucked net.
