# PLAN: Corporate Pricing — net-new module automation (DOCX-first, FCC second, edge third)  [SUPERSEDED — remaining waves re-homed]

> **SUPERSEDED (2026-06-24) by [PLAN_CORP_PRICING_JIRA_DELIVERY.md](PLAN_CORP_PRICING_JIRA_DELIVERY.md).** Waves 0/1/1.5 are DONE and delivered (the milestone annotations below stay valid). The remaining pending tail was re-sequenced into Jira-aligned deliverables + a shadow tier under the new parent (conservation restructure, LR-050): Wave-2 FCC (1441/1443) → folded into NM-2261 / NM-2260; Wave-2.5 dependency-map (PRE_EDGE) → SHADOW_INTEGRATION; Wave-3 edge (EDGE_P3) → SHADOW_EDGE, with the file-I/O round-trips into NM-2262/2264/2305/2265; 1444 History → re-pointed to the new parent (still gated). No work lost. Retained for history.
