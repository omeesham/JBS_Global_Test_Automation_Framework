 ...
 * USED-BY: tests/specs/locations/location-account-address.spec.ts, fixtures.ts
...
export class LocationAccountAddressPage extends BasePage {
  ...
  /** Fill account name filter and click Search. Waits for results to render. */
  async searchAccountByName(name: string): Promise<void> {
    ...
    // Wait for search API to return and table to re-render with actual content
    const table = this.getElement('tblAccListResults');
    await table.locator('tbody tr td:nth-child(2)').first().waitFor({ state: 'visible', timeout: 15_000 });
    // Extra wait for cell content to populate (Angular rendering)
