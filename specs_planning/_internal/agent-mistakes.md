...
| GEN-026 | Post-save state reset: reload before next test. After a save cycle, Angular dirty-state tracking does NOT reset without a full page reload. Always add `reloadAndNavigate()` between tests that modify and save data | Legal TC-012/013 failed: dirty state persisted from prior test's save cycle |
