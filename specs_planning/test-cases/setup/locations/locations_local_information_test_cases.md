...
2. Sort by Modified On descending (SP1 §11: default is ascending on Location Mgmt) -> Rows newest-first
3. Read rows newer than suite start (2-min buffer for SP1 §11 timezone uncertainty) via `getRowsSinceTimestamp` -> Suite rows isolated
...
6. Gap-check (expect.soft, collect all mismatches) expected field-value pairs:
   - Billing Type includes Master or Direct (TC-025 round-trip) -> In history
   - Oracle Product Code includes test values (TC-021/029/068) -> In history
   - Oracle Department Code includes test values (TC-069) -> In history
   - Calculate LDW on Net Amount includes ✔ or empty (TC-021/029 toggle) -> In history
   - Enable IDC Billing includes ✔ or empty (TC-072 toggle) -> In history
7. NOT-TRACKED documented (no column): TC-LOC-LI-071 EnableMultidayPricing — potential app bug per SP1 §8

**Expected**: Each tracked LI save produced 1 new history row. Boolean TRUE = Unicode "checkmark". Timestamps = MM/DD/YYYY HH:MM:SS AM/PM. Percentages = "N.NN %" (space before %). EnableMultidayPricing is NOT tracked → file BUG-HIS-NNN per LR-034 if confirmed.
**Data**: location=1604 | Formats per SUBPLAN_HISTORY_01_MCP_FINDINGS.md sections 1, 3
**Automation File**: tests/specs/setup/locations/location-local-information.spec.ts (TC-LOC-LI-HIST, last in describe.serial)
**Automatable**: Yes

**MCP_VERIFICATION_LOG**:
- Expected: 1 save = 1 new row (snapshot model); col 13 Billing Type; col 48 Enable IDC Billing (Unicode "✔"); col 73 Oracle Product Code; TC-LOC-LI-071 EnableMultidayPricing is NOT-TRACKED (no column in 87)
- Source: SUBPLAN_HISTORY_01_MCP_FINDINGS.md §1 lines 17-107 (87-col header list: col 13 Billing Type, col 48 Enable IDC Billing, col 73 Oracle Product Code all present), §1 lines 109-122 (data formats: Unicode "✔" for boolean TRUE, MM/DD/YYYY HH:MM:SS AM/PM timestamp, "N.NN %" percentage format), §3 lines 259-263 (LOC Union toggle → 1 new row — LOC snapshot model confirmed), §8 lines 336-341 (NOT-TRACKED registry: EnableMultidayPricing explicitly listed)
